...
| [`CMO`](agents/cmo.md) | Chief Marketing Officer who runs paid acquisition end-to-end via Marketing AI plugin (Meta + Google Ads): proposes campaigns, generates creative, submits for human approval, monitors ROAS, pauses underperformers | `claude_local` (Sonnet/Opus has strongest brief and creative judgment) | Medium (workflow-heavy, lens-light) |
...
- **CMO** — the hire is accountable for paid acquisition (Meta Ads + Google Ads) end-to-end via the Marketing AI plugin: proposes campaigns, generates creative, submits for human approval, monitors ROAS, pauses underperformers. Pick CMO when the company has the Marketing AI plugin installed and configured (Meta/Google/Shopify/Anthropic/OpenAI secrets per company). Avoid for content-creator roles (posts, newsletters, blog) — that is a different role; CMO output is **campaigns**, not content. Avoid for companies without paid-ads strategy; an agent calling Meta with no budget cap or brand kit will burn money fast.
