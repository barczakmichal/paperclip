...
export async function fetchInsights(
  ...
  const rows = await cursor.next() as Array<Record<string, unknown>>;
  const row = rows[0];
  ...
  const actionValues = (row["action_values"] as unknown as Array<{ action_type: string; value: string }>) ?? [];
  const conversionValue = actionValues
    .filter((a) => a.action_type === "offsite_conversion.fb_pixel_purchase")
    .reduce((sum, a) => sum + parseFloat(a.value ?? "0"), 0);
  const actions = (row["actions"] as unknown as Array<{ action_type: string; value: string }>) ?? [];
