...
const SOURCE_TYPE_OPTIONS: Array<{ value: ProjectWorkspaceSourceType; labelKey: string; descriptionKey: string }> = [
  { value: "local_path", labelKey: "localGitCheckout", descriptionKey: "localGitCheckoutDesc" },
  { value: "non_git_path", labelKey: "localNonGitPath", descriptionKey: "localNonGitPathDesc" },
  { value: "git_repo", labelKey: "remoteGitRepo", descriptionKey: "remoteGitRepoDesc" },
  { value: "remote_managed", labelKey: "remoteManagedWorkspace", descriptionKey: "remoteManagedWorkspaceDesc" },
];
...
function parseRuntimeConfigJson(value: string, t: (key: string) => string) {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false as const,
        error: t("page.projectWorkspaceDetail.validation.commandsJsonMustBeObject"),
      };
    }
    ...
    return {
      ok: false as const,
      error: error instanceof Error ? error.message : t("page.projectWorkspaceDetail.validation.invalidJson"),
    };
  }
}
...
function validateWorkspaceForm(form: WorkspaceFormState, t: (key: string) => string) {
  ...
  if (form.sourceType === "remote_managed") {
    if (!remoteWorkspaceRef && !repoUrl) {
      return t("page.projectWorkspaceDetail.validation.remoteManagedRequiresRefOrUrl");
    }
  } else if (!cwd && !repoUrl) {
    return t("page.projectWorkspaceDetail.validation.requiresPathOrUrl");
  }

  if (cwd && (form.sourceType === "local_path" || form.sourceType === "non_git_path") && !isAbsolutePath(cwd)) {
    return t("page.projectWorkspaceDetail.validation.localPathMustBeAbsolute");
  }
  ...
  if (repoUrl) {
    try {
      ...
      return t("page.projectWorkspaceDetail.validation.repoUrlMustBeValid");
    }
  }
...
export function ProjectWorkspaceDetail() {
  ...
  useEffect(() => {
    ...
    setBreadcrumbs([
      ...
      { label: t("page.project.breadcrumb.workspaces"), href: `/projects/${canonicalProjectRef}/workspaces` },
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("page.projectWorkspaceDetail.error.saveFailed"));
    },
  });
  ...
  const setPrimaryWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("page.projectWorkspaceDetail.error.updateFailed"));
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, request) => {
      ...
      setRuntimeActionMessage(
        request.action === "run"
          ? t("page.projectWorkspaceDetail.runtimeAction.jobCompleted")
          : request.action === "stop"
            ? "Workspace service stopped. Task execution is not paused."
            : request.action === "restart"
              ? "Workspace service restarted. Task execution is not paused."
              : t("page.projectWorkspaceDetail.runtimeAction.serviceStarted"),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setErrorMessage(error instanceof Error ? error.message : t("page.projectWorkspaceDetail.error.controlFailed"));
    },
  });

  if (projectQuery.isLoading) return <p className="text-sm text-muted-foreground">{t("page.projectWorkspaceDetail.loading")}</p>;
  if (projectQuery.error) {
    return (
      <p className="text-sm text-destructive">
        {projectQuery.error instanceof Error ? projectQuery.error.message : t("page.projectWorkspaceDetail.error.loadFailed")}
      </p>
    );
  }
  if (!project || !workspace || !form || !initialState) {
    return <p className="text-sm text-muted-foreground">{t("page.projectWorkspaceDetail.notFound")}</p>;
  }
  ...
  const sourceTypeDescription = SOURCE_TYPE_OPTIONS.find((option) => option.value === form.sourceType) ? t(`page.projectWorkspaceDetail.sourceType.${SOURCE_TYPE_OPTIONS.find((option) => option.value === form.sourceType)?.descriptionKey ?? ""}`) : null;
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Button variant="ghost" size="sm" asChild>
          <Link to={`/projects/${canonicalProjectRef}/workspaces`}>
            ...
            {t("page.projectWorkspaceDetail.backToWorkspaces")}
          </Link>
        </Button>
      </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Field label={t("page.projectWorkspaceDetail.field.workspaceName")}>
                <input
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                  value={form.name}
                  onChange={(event) => setForm((current) => current ? { ...current, name: event.target.value } : current)}
                  placeholder={t("page.projectWorkspaceDetail.field.workspaceName")}
                />
              </Field>

              <Field label={t("page.projectWorkspaceDetail.field.visibility")}>
                <select
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                ...
                >
                  {VISIBILITY_OPTIONS.map((option) => (
                    <option key={option.value} value={option.value}>{t(`page.projectWorkspaceDetail.visibility.${option.labelKey}`)}</option>
                  ))}
                </select>
              </Field>
            </div>
            ...
            <div className="mt-4 grid gap-4">
              <Field label={t("page.projectWorkspaceDetail.field.sourceType")} hint={sourceTypeDescription ?? undefined}>
                <select
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                ...
                >
                  {SOURCE_TYPE_OPTIONS.map((option) => (
                    <option key={option.value} value={option.value}>{t(`page.projectWorkspaceDetail.sourceType.${option.labelKey}`)}</option>
                  ))}
                </select>
              </Field>

              <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_auto]">
                <Field label={t("page.projectWorkspaceDetail.field.localPath")}>
                  <input
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("page.projectWorkspaceDetail.field.repoUrl")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                    value={form.repoUrl}
                ...
                <Field label={t("page.projectWorkspaceDetail.field.repoRef")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    value={form.repoRef}
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("page.projectWorkspaceDetail.field.defaultRef")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                ...
                <Field label={t("page.projectWorkspaceDetail.field.sharedWorkspaceKey")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("page.projectWorkspaceDetail.field.remoteProvider")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                ...
                <Field label={t("page.projectWorkspaceDetail.field.remoteWorkspaceRef")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("page.projectWorkspaceDetail.field.setupCommand")} hint={t("page.projectWorkspaceDetail.hint.setupCommand")}>
                  <textarea
                ...
                <Field label={t("page.projectWorkspaceDetail.field.cleanupCommand")} hint={t("page.projectWorkspaceDetail.hint.cleanupCommand")}>
                  <textarea
                    className="min-h-28 w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <details className="rounded-xl border border-dashed border-border/70 bg-background px-3 py-3">
                <summary className="cursor-pointer text-sm font-medium">{t("page.projectWorkspaceDetail.advancedRuntimeJson")}</summary>
                <p className="mt-2 text-sm text-muted-foreground">
                  {t("page.projectWorkspaceDetail.advancedRuntimeJsonDesc")}
                </p>
                <div className="mt-3">
                  <Field label={t("page.projectWorkspaceDetail.field.workspaceCommandsJson")} hint={t("page.projectWorkspaceDetail.hint.workspaceCommandsJson")}>
                    <textarea
            ...
            <div className="mt-5 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
              <Button className="w-full sm:w-auto" disabled={!isDirty || updateWorkspace.isPending} onClick={saveChanges}>
                ...
                {t("page.projectWorkspaceDetail.saveChanges")}
              </Button>
              <Button
                variant="outline"
                className="w-full sm:w-auto"
              ...
              >
                {t("page.projectWorkspaceDetail.reset")}
              </Button>
              {errorMessage ? <p className="text-sm text-destructive">{errorMessage}</p> : null}
              {!errorMessage && runtimeActionMessage ? <p className="text-sm text-muted-foreground">{runtimeActionMessage}</p> : null}
              {!errorMessage && !isDirty ? <p className="text-sm text-muted-foreground">{t("page.projectWorkspaceDetail.noUnsavedChanges")}</p> : null}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="space-y-1">
              <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("page.projectWorkspaceDetail.workspaceFacts")}</div>
              <h2 className="text-lg font-semibold">{t("page.projectWorkspaceDetail.currentState")}</h2>
            </div>
            <Separator className="my-4" />
            <DetailRow label={t("page.projectWorkspaceDetail.detail.project")}>
              <Link to={`/projects/${canonicalProjectRef}`} className="hover:underline">{project.name}</Link>
            </DetailRow>
            <DetailRow label={t("page.projectWorkspaceDetail.detail.workspaceId")}>
              <span className="break-all font-mono text-xs">{workspace.id}</span>
            </DetailRow>
            <DetailRow label={t("page.projectWorkspaceDetail.detail.localPath")}>
              <span className="break-all font-mono text-xs">{workspace.cwd ?? t("page.projectWorkspaceDetail.none")}</span>
            </DetailRow>
            <DetailRow label={t("page.projectWorkspaceDetail.detail.repo")}>
              ...
              ) : t("page.projectWorkspaceDetail.none")}
            </DetailRow>
            <DetailRow label={t("page.projectWorkspaceDetail.detail.defaultRef")}>{workspace.defaultRef ?? t("page.projectWorkspaceDetail.none")}</DetailRow>
            <DetailRow label={t("page.projectWorkspaceDetail.detail.updated")}>{new Date(workspace.updatedAt).toLocaleString()}</DetailRow>
          </div>

          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-1">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("page.projectWorkspaceDetail.workspaceCommands")}</div>
                <h2 className="text-lg font-semibold">{t("page.projectWorkspaceDetail.servicesAndJobs")}</h2>
                <p className="text-sm text-muted-foreground">
                  {t("page.projectWorkspaceDetail.servicesAndJobsDesc")}
                </p>
              </div>
            </div>
            <WorkspaceRuntimeControls
              ...
              serviceEmptyMessage={
                workspace.runtimeConfig?.workspaceRuntime
                  ? t("page.projectWorkspaceDetail.empty.servicesNotStarted")
                  : t("page.projectWorkspaceDetail.empty.noCommandConfig")
              }
              jobEmptyMessage={t("page.projectWorkspaceDetail.empty.noJobs")}
              disabledHint={t("page.projectWorkspaceDetail.disabledHint")}
