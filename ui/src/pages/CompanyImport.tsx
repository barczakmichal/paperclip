...
function ImportPreviewPane({
  ...
  const { t } = useTranslation("companyImportPage");
      ...
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          <h3 className="text-sm font-medium">
            {t("renamesHeading", "Renames")}
          </h3>
          <span className="text-xs text-muted-foreground">
            {t("itemsCount", "{{count}} item", { count: conflicts.length })}
          </span>
        </div>
          ...
          {conflicts.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isSkipped ? t("skipped", "skipped") : t("skip", "skip")}
                </button>
                ...
                {!isSkipped && (
                  ...
                  >
                    {isConfirmed ? (
                      ...
                      t("confirmRename", "confirm rename")
                    )}
                  </button>
                )}
              </div>
...
function AdapterPickerList({
  ...
  return (
    <div className="mx-5 mt-3">
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          <h3 className="text-sm font-medium">{t("adaptersHeading", "Adapters")}</h3>
          <span className="text-xs text-muted-foreground">
            {t("agentsCount", "{{count}} agent", { count: agents.length })}
          </span>
        </div>
        <div className="divide-y divide-border">
          {agents.map((agent) => {
              ...
              <div key={agent.slug}>
                <div className="flex items-center gap-3 px-4 py-2.5 text-sm">
                  ...
                  >
                    ...
                    {t("configureAdapter", "configure adapter")}
                  </button>
                </div>
                {isExpanded && (
...
async function readLocalPackageZip(file: File): Promise<{
  ...
  if (!/\.zip$/i.test(file.name)) {
    throw new Error(i18n.t("companyImportPage:errorSelectZip", "Select a .zip company package."));
  }
  const archive = await readZipArchive(await file.arrayBuffer());
  if (Object.keys(archive.files).length === 0) {
    throw new Error(i18n.t("companyImportPage:errorNoPackageFiles", "No package files were found in the selected zip archive."));
  }
  return {
    name: file.name,
...
export function CompanyImport() {
  ...
  const localZipHelpText = t("localZipHelp", "Upload a .zip exported directly from Paperclip. Re-zipped archives created by Finder, Explorer, or other zip tools may not import correctly.");

  useEffect(() => {
    setBreadcrumbs([
      { label: t("orgChartBreadcrumb", "Org Chart"), href: "/org" },
      { label: t("importBreadcrumb", "Import") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const previewMutation = useMutation({
    mutationFn: () => {
      ...
      if (!source) throw new Error(t("noSourceConfigured", "No source configured."));
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("toastPreviewFailed", "Preview failed"),
        body: err instanceof Error ? err.message : t("previewImportFailed", "Failed to preview import."),
      });
    },
  });
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("toastImportComplete", "Import complete"),
        body: t("toastImportProcessed", "{{name}}: {{count}} agent processed.", { name: result.company.name, count: result.agents.length }),
      });
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("toastImportFailed", "Import failed"),
        body: err instanceof Error ? err.message : t("applyImportFailed", "Failed to apply import."),
      });
    },
  });
  ...
  async function handleChooseLocalPackage(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      pushToast({
        tone: "error",
        title: t("toastPackageReadFailed", "Package read failed"),
        body: err instanceof Error ? err.message : t("readFolderFailed", "Failed to read folder."),
      });
    }
  }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Download} message={t("selectCompany", "Select a company to import into.")} />;
  }
  ...
  return (
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        <div>
          <h2 className="text-base font-semibold">{t("importSourceHeading", "Import source")}</h2>
          <p className="text-xs text-muted-foreground mt-1">
            {t("importSourceSubtitle", "Choose a GitHub repo or upload a local Paperclip zip package.")}
          </p>
        </div>
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {(
            [
              { key: "github", icon: Github, label: t("sourceGithub", "GitHub repo") },
              { key: "local", icon: Upload, label: t("sourceLocal", "Local zip") },
            ] as const
        ...
        {sourceMode === "local" ? (
          <div className="rounded-md border border-dashed border-border px-3 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {t("chooseZip", "Choose zip")}
              </Button>
              {localPackage && (
                <span className="text-xs text-muted-foreground">
                  {t("packageWithFiles", "{{name}} with {{count}} file", { name: localPackage.name, count: Object.keys(localPackage.files).length })}
                </span>
              )}
            </div>
            {!localPackage && (
          ...
          <Field
            label={t("githubUrlLabel", "GitHub URL")}
            hint={t("githubUrlHint", "Repo tree path or blob URL to COMPANY.md (e.g. github.com/owner/repo/tree/main/company).")}
          >
            <input
        ...
        <Field label={t("targetLabel", "Target")} hint={t("targetHint", "Import into this company or create a new one.")}>
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {
          ...
          >
            <option value="new">{t("createNewCompany", "Create new company")}</option>
            <option value="existing">
              {t("existingCompany", "Existing company: {{name}}", { name: selectedCompany?.name })}
            </option>
          </select>
        </Field>
        ...
        {targetMode === "new" && (
          <Field
            label={t("newCompanyNameLabel", "New company name")}
            hint={t("newCompanyNameHint", "Optional override. Leave blank to use the package name.")}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              ...
              placeholder={t("importedCompanyPlaceholder", "Imported Company")}
            />
          </Field>
        )}

        <Field
          label={t("collisionStrategyLabel", "Collision strategy")}
          hint={t("collisionStrategyHint", "Board imports can rename, skip, or replace matching company content.")}
        >
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
          ...
          >
            <option value="rename">{t("renameOnConflict", "Rename on conflict")}</option>
            <option value="skip">{t("skipOnConflict", "Skip on conflict")}</option>
            <option value="replace">{t("replaceExisting", "Replace existing")}</option>
          </select>
        </Field>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {previewMutation.isPending ? t("previewing", "Previewing...") : t("previewImport", "Preview import")}
          </Button>
        </div>
      </div>
          ...
          <div className="sticky top-0 z-10 border-b border-border bg-background px-5 py-3">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              <span className="font-medium">
                {t("importPreviewLabel", "Import preview")}
              </span>
              ...
              {conflicts.length > 0 && (
                <span className="text-amber-500">
                  {t("conflictsCount", "{{count}} conflict", { count: conflicts.length })}
                </span>
              )}
              {importPreview.errors.length > 0 && (
                <span className="text-destructive">
                  {t("errorsCount", "{{count}} error", { count: importPreview.errors.length })}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="mx-5 mt-3 flex justify-end">
            ...
            >
              ...
              {importMutation.isPending
                ...
                : t("importFiles", "Import {{count}} file", { count: selectedCount })}
            </Button>
          </div>
