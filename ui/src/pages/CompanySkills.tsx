...
function sourceMeta(sourceBadge: CompanySkillSourceBadge, sourceLabel: string | null, t: (key: string, defaultValue: string) => string) {
  ...
  switch (sourceBadge) {
    case "skills_sh":
      return { icon: VercelMark, label: sourceLabel ?? "skills.sh", managedLabel: t("managedSkillsSh", "skills.sh managed") };
    case "github":
      return isSkillsShManaged
        ? { icon: VercelMark, label: sourceLabel ?? "skills.sh", managedLabel: t("managedSkillsSh", "skills.sh managed") }
        : { icon: Github, label: sourceLabel ?? "GitHub", managedLabel: t("managedGithub", "GitHub managed") };
    case "url":
      return { icon: Link2, label: sourceLabel ?? "URL", managedLabel: t("managedUrl", "URL managed") };
    case "local":
      return { icon: Folder, label: sourceLabel ?? t("sourceFolder", "Folder"), managedLabel: t("managedFolder", "Folder managed") };
    case "paperclip":
      return { icon: Paperclip, label: sourceLabel ?? "Paperclip", managedLabel: t("managedPaperclip", "Paperclip managed") };
    default:
      return { icon: Boxes, label: sourceLabel ?? t("sourceCatalog", "Catalog"), managedLabel: t("managedCatalog", "Catalog managed") };
  }
}
...
function formatProjectScanSummary(result: CompanySkillProjectScanResult, t: (key: string, defaultValue: string, options?: Record<string, unknown>) => string) {
  const parts = [
    t("scanFound", "{{count}} found", { count: result.discovered }),
    t("scanImported", "{{count}} imported", { count: result.imported.length }),
    t("scanUpdated", "{{count}} updated", { count: result.updated.length }),
  ];
  if (result.conflicts.length > 0) parts.push(t("scanConflicts", "{{count}} conflicts", { count: result.conflicts.length }));
  if (result.skipped.length > 0) parts.push(t("scanSkipped", "{{count}} skipped", { count: result.skipped.length }));
  return t("scanSummary", "{{parts}} across {{count}} workspace.", { parts: parts.join(", "), count: result.scannedWorkspaces });
}
...
function NewSkillForm({
  ...
  const { t } = useTranslation("companySkillsPage");
    ...
    <div className="border-b border-border px-4 py-4">
      <div className="space-y-3">
        ...
        <Input
          ...
          placeholder={t("slugPlaceholder", "optional-shortname")}
          className="h-9 rounded-none border-0 border-b border-border px-0 shadow-none focus-visible:ring-0"
        />
        <Textarea
          value={description}
          onChange={(event) => setDescription(event.target.value)}
          placeholder={t("shortDescriptionPlaceholder", "Short description")}
        ...
        <div className="flex items-center justify-end gap-2">
          ...
          >
            {isPending ? t("creating", "Creating...") : t("createSkill", "Create skill")}
          </Button>
        </div>
      </div>
    </div>
  ...
  if (filteredSkills.length === 0) {
    return (
      <div className="px-4 py-6 text-sm text-muted-foreground">
        {t("noSkillsMatch", "No skills match this filter.")}
      </div>
    );
  }
          ...
          <div key={skill.id} className="border-b border-border">
            ...
            >
              ...
              <button
                ...
                aria-label={expanded ? t("collapseSkill", "Collapse {{name}}", { name: skill.name }) : t("expandSkill", "Expand {{name}}", { name: skill.name })}
              >
                {expanded ? <ChevronDown className="h-3.5 w-3.5" /> : <ChevronRight className="h-3.5 w-3.5" />}
              </button>
            </div>
...
function SkillPane({
  ...
  if (!detail) {
    ...
    return (
      <EmptyState
        icon={Boxes}
        message={t("selectSkillInspect", "Select a skill to inspect its files.")}
      />
    );
  }
  ...
  const removeDisabledReason = removeBlocked
    ? t("detachBeforeRemove", "Detach this skill from all agents before removing it.")
    : null;
    ...
    <div className="min-w-0">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            >
              ...
              {deletePending ? t("removing", "Removing...") : t("remove", "Remove")}
            </Button>
            {detail.editable ? (
              <button
              ...
              >
                ...
                {editMode ? t("stopEditing", "Stop editing") : t("edit", "Edit")}
              </button>
            ) : (
        ...
        <div className="mt-4 space-y-3 border-t border-border pt-4 text-sm">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
            <div className="flex items-center gap-2">
              <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("sourceLabel", "Source")}</span>
                ...
                {detail.sourcePath ? (
                  <button
                    ...
                    onClick={() => {
                      ...
                      pushToast({ title: t("copiedPath", "Copied path to workspace") });
                    }}
                  >
                    {source.label}
                  </button>
            ...
            {detail.sourceType === "github" && (
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("pinLabel", "Pin")}</span>
                <span className="font-mono text-xs">{currentPin ?? t("untracked", "untracked")}</span>
                {updateStatus?.trackingRef && (
                  <span className="text-xs text-muted-foreground">{t("tracking", "tracking {{ref}}", { ref: updateStatus.trackingRef })}</span>
                )}
                <Button
                  variant="ghost"
                  size="sm"
                ...
                {updateStatus?.supported && updateStatus.hasUpdate && (
                  ...
                  >
                    ...
                    {t("installUpdate", "Install update")}{latestPin ? ` ${latestPin}` : ""}
                  </Button>
                )}
                {updateStatus?.supported && !updateStatus.hasUpdate && !updateStatusLoading && (
                  <span className="text-xs text-muted-foreground">{t("upToDate", "Up to date")}</span>
                )}
            ...
            <div className="flex items-center gap-2">
              <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("keyLabel", "Key")}</span>
              <span className="font-mono text-xs">{detail.key}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("modeLabel", "Mode")}</span>
              <span>{detail.editable ? t("editable", "Editable") : t("readOnly", "Read only")}</span>
            </div>
          </div>
          <div className="flex flex-wrap items-start gap-x-3 gap-y-1">
            <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("usedByLabel", "Used by")}</span>
            {usedBy.length === 0 ? (
              <span className="text-muted-foreground">{t("noAgentsAttached", "No agents attached")}</span>
            ) : (
            ...
            {editMode && file?.editable && (
              <>
                ...
                <Button size="sm" onClick={onSave} disabled={savePending}>
                  ...
                  {savePending ? t("saving", "Saving...") : t("save", "Save")}
                </Button>
              </>
            )}
          </div>
      ...
      <div className="min-h-[560px] px-5 py-5">
        {fileLoading ? (
          ...
          <div className="text-sm text-muted-foreground">{t("selectFileInspect", "Select a file to inspect.")}</div>
        ) : editMode && file.editable ? (
          file.markdown ? (
            <MarkdownEditor
              value={draft}
...
export function CompanySkills() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("skillsBreadcrumb", "Skills"), href: "/skills" },
      ...(routeSkillId ? [{ label: t("detailBreadcrumb", "Detail") }] : []),
    ]);
  ...
  const importSkill = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("toastSkillsImported", "Skills imported"),
        body: t("toastSkillsAdded", "{{count}} skill added.", { count: result.imported.length }),
      });
      if (result.warnings[0]) {
        pushToast({ tone: "warn", title: t("toastImportWarnings", "Import warnings"), body: result.warnings[0] });
      }
      setSource("");
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("toastSkillImportFailed", "Skill import failed"),
        body: error instanceof Error ? error.message : t("importSourceFailed", "Failed to import skill source."),
      });
    },
  });
  ...
  const createSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: t("toastSkillCreated", "Skill created"),
        body: t("toastSkillEditable", "{{name}} is now editable in the Paperclip workspace.", { name: skill.name }),
      });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("toastSkillCreateFailed", "Skill creation failed"),
        body: error instanceof Error ? error.message : t("createSkillFailed", "Failed to create skill."),
      });
    },
  });
  ...
  const scanProjects = useMutation({
    ...
    onMutate: () => {
      setScanStatusMessage(t("scanning", "Scanning project workspaces for skills..."));
    },
    onSuccess: async (result) => {
      setScanStatusMessage(t("refreshingList", "Refreshing skills list..."));
      await queryClient.invalidateQueries({ queryKey: queryKeys.companySkills.list(selectedCompanyId!) });
      const summary = formatProjectScanSummary(result, t);
      setScanStatusMessage(summary);
      pushToast({
        tone: "success",
        title: t("toastScanComplete", "Project skill scan complete"),
        body: summary,
      });
      if (result.conflicts[0]) {
        pushToast({
          tone: "warn",
          title: t("toastConflictsFound", "Skill conflicts found"),
          body: result.conflicts[0].reason,
        });
      } else if (result.warnings[0]) {
        pushToast({
          tone: "warn",
          title: t("toastScanWarnings", "Scan warnings"),
    ...
    onError: (error) => {
      ...
      pushToast({
        tone: "error",
        title: t("toastScanFailed", "Project skill scan failed"),
        body: error instanceof Error ? error.message : t("scanFailed", "Failed to scan project workspaces."),
      });
    },
  });
  ...
  const saveFile = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("toastSkillSaved", "Skill saved"),
        body: result.path,
      });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("toastSaveFailed", "Save failed"),
        body: error instanceof Error ? error.message : t("saveFileFailed", "Failed to save skill file."),
      });
    },
  });
  ...
  const installUpdate = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: t("toastSkillUpdated", "Skill updated"),
        body: skill.sourceRef ? t("pinnedTo", "Pinned to {{ref}}", { ref: shortRef(skill.sourceRef) }) : skill.name,
      });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("toastUpdateFailed", "Update failed"),
        body: error instanceof Error ? error.message : t("installUpdateFailed", "Failed to install skill update."),
      });
    },
  });
  ...
  const deleteSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: t("toastSkillRemoved", "Skill removed"),
        body: t("toastSkillRemovedBody", "{{name}} was removed from the company skill library.", { name: skill.name }),
      });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("toastRemoveFailed", "Remove failed"),
        body: error instanceof Error ? error.message : t("removeSkillFailed", "Failed to remove skill."),
      });
    },
  });

  if (!selectedCompanyId) {
    return <EmptyState icon={Boxes} message={t("selectCompany", "Select a company to manage skills.")} />;
  }
    ...
    <>
      <Dialog open={deleteOpen} onOpenChange={closeDeleteDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("removeSkillTitle", "Remove skill")}</DialogTitle>
            <DialogDescription>
              {t("removeSkillDescription", "Remove this skill from the company library. If any agents still use it, removal will be blocked until it is detached.")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            <p>
              {deleteTargetDetail
                ? t("aboutToRemoveNamed", "You are about to remove {{name}}.", { name: deleteTargetDetail.name })
                : t("aboutToRemove", "You are about to remove this skill.")}
            </p>
            {deleteTargetDetail?.usedByAgents?.length ? (
              <div className="rounded-md border border-border px-3 py-3 text-muted-foreground">
                {t("currentlyUsedBy", "Currently used by {{agents}}.", { agents: deleteTargetDetail.usedByAgents.map((agent) => agent.name).join(", ") })}
              </div>
            ) : null}
            {(deleteTargetDetail?.usedByAgents.length ?? 0) > 0 ? (
              <p className="text-muted-foreground">
                {t("detachToEnableRemoval", "Detach this skill from all agents to enable removal.")}
              </p>
            ) : null}
          </div>
          <DialogFooter>
            {(deleteTargetDetail?.usedByAgents.length ?? 0) > 0 ? (
              ...
              <>
                ...
                >
                  {deleteSkill.isPending ? t("removing", "Removing...") : t("removeSkill", "Remove skill")}
                </Button>
              </>
            )}
          </DialogFooter>
      ...
      <Dialog open={emptySourceHelpOpen} onOpenChange={setEmptySourceHelpOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("addSourceTitle", "Add a skill source")}</DialogTitle>
            <DialogDescription>
              {t("addSourceDescription", "Paste a local path, GitHub URL, or `skills.sh` command into the field first.")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            <a
            ...
            >
              <span>
                <span className="block font-medium">{t("browseSkillsSh", "Browse skills.sh")}</span>
                <span className="mt-1 block text-muted-foreground">
                  {t("browseSkillsShHint", "Find install commands and paste one here.")}
                </span>
              </span>
            ...
            >
              <span>
                <span className="block font-medium">{t("searchGithub", "Search GitHub")}</span>
                <span className="mt-1 block text-muted-foreground">
                  {t("searchGithubHint", "Look for repositories with `SKILL.md`, then paste the repo URL here.")}
                </span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
        ...
        <aside className="border-r border-border">
          <div className="border-b border-border px-4 py-3">
            <div className="flex items-center justify-between gap-2">
              <div>
                <h1 className="text-base font-semibold">{t("skillsHeading", "Skills")}</h1>
                <p className="text-xs text-muted-foreground">
                  {t("available", "{{count}} available", { count: skillsQuery.data?.length ?? 0 })}
                </p>
              </div>
              <div className="flex items-center gap-1">
                <Button
                  ...
                  title={t("scanTooltip", "Scan project workspaces for skills")}
                >
            ...
            <div className="mt-3 flex items-center gap-2 border-b border-border pb-2">
              ...
              <input
                ...
                placeholder={t("filterSkills", "Filter skills")}
            ...
            <div className="mt-3 flex items-center gap-2 border-b border-border pb-2">
              <input
                ...
                placeholder={t("sourcePlaceholder", "Paste path, GitHub URL, or skills.sh command")}
                className="w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground"
              />
              <Button
                size="sm"
              ...
              >
                {importSkill.isPending ? <RefreshCw className="h-4 w-4 animate-spin" /> : t("add", "Add")}
              </Button>
            </div>
            {scanStatusMessage && (
