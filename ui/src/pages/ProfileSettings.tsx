...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("nav.instanceSidebar.profile") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("page.profileSettings.errors.updateFailed"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("page.profileSettings.errors.selectCompanyForAvatar"));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("page.profileSettings.errors.avatarUploadFailed"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("page.profileSettings.errors.avatarRemoveFailed"));
    },
  });

  if (sessionQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("page.profileSettings.loading")}</div>;
  }

  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("page.profileSettings.errors.loadFailed")}
      </div>
    );
  }
  ...
  const uploadHint = selectedCompany
    ? t("page.profileSettings.uploadHint.stored", { companyName: selectedCompany.name })
    : t("page.profileSettings.uploadHint.selectCompany");
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("page.profileSettings.title")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("page.profileSettings.description")}
        </p>
      </div>
      ...
      <section className="space-y-8">
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-[24px] border border-border/70 bg-background/92 p-5 shadow-[0_18px_44px_-28px_rgba(0,0,0,0.45)] backdrop-blur-sm">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {uploadAvatarMutation.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Camera className="size-4" />}
                    {currentImage ? t("page.profileSettings.changePhoto") : t("page.profileSettings.uploadPhoto")}
                  </Button>
                  {currentImage ? (
                    <Button
                      type="button"
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                <div>
                  ...
                  <p className="truncate text-sm text-muted-foreground">{sessionQuery.data.user.email ?? t("page.profileSettings.noEmail")}</p>
                </div>
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("page.profileSettings.avatarUploadHint")} {uploadHint}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-2">
            <Label htmlFor="profile-name">{t("page.profileSettings.displayName")}</Label>
            <Input
              id="profile-name"
              value={name}
              onChange={(event) => setName(event.target.value)}
              maxLength={120}
              placeholder={t("page.profileSettings.placeholder.board")}
            />
            <p className="text-xs text-muted-foreground">
              {t("page.profileSettings.displayNameHint")}
            </p>
          </div>

          <div className="space-y-2">
            <Label htmlFor="profile-email">{t("common.form.email", { defaultValue: "Email" })}</Label>
            <Input
              id="profile-email"
            ...
            />
            <p className="text-xs text-muted-foreground">
              {t("page.profileSettings.emailHint")}
            </p>
          </div>
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending ? t("common.actions.saving") : t("page.profileSettings.saveProfile")}
            </Button>
          </div>
        </form>
      </section>
