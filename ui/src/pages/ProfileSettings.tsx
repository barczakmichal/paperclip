...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("profileBreadcrumb", "Profile") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("updateProfileFailed", "Failed to update profile."));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("selectCompanyBeforeUpload", "Select a company before uploading a profile avatar."));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("uploadAvatarFailed", "Failed to upload avatar."));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("removeAvatarFailed", "Failed to remove avatar."));
    },
  });

  if (sessionQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("loadingProfile", "Loading profile...")}</div>;
  }

  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("loadProfileFailed", "Failed to load profile.")}
      </div>
    );
  }

  const currentName = name.trim() || sessionQuery.data.user.name || t("boardFallback", "Board");
  ...
  const uploadHint = selectedCompany
    ? t("uploadHintStored", "Stored in Paperclip file storage for {{company}}.", { company: selectedCompany.name })
    : t("uploadHintSelectCompany", "Select a company to upload an avatar into Paperclip storage.");
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("title", "Profile")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("subtitle", "Control how your account appears in the sidebar and other board surfaces.")}
        </p>
      </div>
      ...
      <section className="space-y-8">
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-[24px] border border-border/70 bg-background/92 p-5 shadow-[0_18px_44px_-28px_rgba(0,0,0,0.45)] backdrop-blur-sm">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {uploadAvatarMutation.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Camera className="size-4" />}
                    {currentImage ? t("changePhoto", "Change photo") : t("uploadPhoto", "Upload photo")}
                  </Button>
                  {currentImage ? (
                    <Button
                      type="button"
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                <div>
                  ...
                  <p className="truncate text-sm text-muted-foreground">{sessionQuery.data.user.email ?? t("noEmail", "No email")}</p>
                </div>
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("clickAvatar", "Click the avatar to upload a new image.")} {uploadHint}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-2">
            <Label htmlFor="profile-name">{t("displayNameLabel", "Display name")}</Label>
            <Input
              id="profile-name"
              value={name}
              onChange={(event) => setName(event.target.value)}
              maxLength={120}
              placeholder={t("displayNamePlaceholder", "Board")}
            />
            <p className="text-xs text-muted-foreground">
              {t("displayNameHint", "Shown in the sidebar account footer and comment author surfaces.")}
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              {t("emailHint", "Email is managed by your auth session and is read-only here.")}
            </p>
          </div>
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending ? t("saving", "Saving...") : t("saveProfile", "Save profile")}
            </Button>
          </div>
        </form>
      </section>
