...
function TriggerEditor({
  ...
  const { t } = useTranslation("routineDetailPage");
    ...
    <div className="rounded-lg border border-border p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">
          {trigger.kind === "schedule" && trigger.nextRunAt
            ? t("nextRun", "Next: {{date}}", { date: new Date(trigger.nextRunAt).toLocaleString() })
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        {trigger.kind === "webhook" && (
          <>
            <div className="space-y-1.5">
              <Label className="text-xs">{t("signingMode", "Signing mode")}</Label>
              <Select
            ...
            {!SIGNING_MODES_WITHOUT_REPLAY_WINDOW.has(draft.signingMode) && (
              <div className="space-y-1.5">
                <Label className="text-xs">{t("replayWindow", "Replay window (seconds)")}</Label>
                <Input
      ...
      <div className="flex flex-wrap items-center gap-2">
        {trigger.lastResult && <span className="text-xs text-muted-foreground">{t("last", "Last: {{result}}", { result: trigger.lastResult })}</span>}
        <div className="ml-auto flex items-center gap-2">
          {trigger.kind === "webhook" && (
            <Button variant="outline" size="sm" onClick={() => onRotate(trigger.id)}>
              <RefreshCw className="mr-1.5 h-3.5 w-3.5" />
              {t("rotateSecret", "Rotate secret")}
            </Button>
          )}
          <Button
            variant="outline"
...
export function RoutineDetail() {
  ...
  useEffect(() => {
    if (!routine) return;
    setBreadcrumbs([{ label: t("breadcrumbRoutines", "Routines"), href: "/routines" }, { label: routine.title }]);
  ...
  const copySecretValue = async (label: string, value: string) => {
    try {
      ...
      pushToast({ title: t("copied", "{{label}} copied", { label }), tone: "success" });
    } catch (error) {
      pushToast({
        title: t("copyFailed", "Failed to copy {{label}}", { label: label.toLowerCase() }),
        body: error instanceof Error ? error.message : t("clipboardDenied", "Clipboard access was denied."),
        tone: "error",
      });
    }
  };
  ...
  const saveRoutine = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("toastSaveFailed", "Failed to save routine"),
        body: error instanceof Error ? error.message : t("toastSaveFailedBody", "Paperclip could not save the routine."),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async () => {
      pushToast({ title: t("toastRunStarted", "Routine run started"), tone: "success" });
    ...
    onError: (error) => {
      pushToast({
        title: t("toastRunFailed", "Routine run failed"),
        body: error instanceof Error ? error.message : t("toastRunFailedBody", "Paperclip could not start the routine run."),
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onSuccess: async (_data, status) => {
      pushToast({
        title: t("toastRoutineSaved", "Routine saved"),
        body: status === "paused" ? t("toastAutomationPaused", "Automation paused.") : t("toastAutomationEnabled", "Automation enabled."),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (error) => {
      pushToast({
        title: t("toastUpdateFailed", "Failed to update routine"),
        body: error instanceof Error ? error.message : t("toastUpdateFailedBody", "Paperclip could not update the routine."),
        tone: "error",
      });
    },
  });
  ...
  const createTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      if (result.secretMaterial) {
        setSecretMessage({
          title: t("secretWebhookCreated", "Webhook trigger created"),
          webhookUrl: result.secretMaterial.webhookUrl,
          webhookSecret: result.secretMaterial.webhookSecret,
        });
      } else {
        pushToast({
          title: t("toastTriggerAdded", "Trigger added"),
          body: t("toastTriggerAddedBody", "The routine schedule was saved."),
          tone: "success",
        });
      }
      await Promise.all([
    ...
    onError: (error) => {
      pushToast({
        title: t("toastAddTriggerFailed", "Failed to add trigger"),
        body: error instanceof Error ? error.message : t("toastAddTriggerFailedBody", "Paperclip could not create the trigger."),
        tone: "error",
      });
    },
  });
  ...
  const updateTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({
        title: t("toastTriggerSaved", "Trigger saved"),
        body: t("toastTriggerSavedBody", "The routine cadence update was saved."),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (error) => {
      pushToast({
        title: t("toastUpdateTriggerFailed", "Failed to update trigger"),
        body: error instanceof Error ? error.message : t("toastUpdateTriggerFailedBody", "Paperclip could not update the trigger."),
        tone: "error",
      });
    },
  });
  ...
  const deleteTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({
        title: t("toastTriggerDeleted", "Trigger deleted"),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (error) => {
      pushToast({
        title: t("toastDeleteTriggerFailed", "Failed to delete trigger"),
        body: error instanceof Error ? error.message : t("toastDeleteTriggerFailedBody", "Paperclip could not delete the trigger."),
        tone: "error",
      });
    },
  });
  ...
  const rotateTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      setSecretMessage({
        title: t("secretWebhookRotated", "Webhook secret rotated"),
        webhookUrl: result.secretMaterial.webhookUrl,
        webhookSecret: result.secretMaterial.webhookSecret,
      });
      await Promise.all([
    ...
    onError: (error) => {
      pushToast({
        title: t("toastRotateFailed", "Failed to rotate webhook secret"),
        body: error instanceof Error ? error.message : t("toastRotateFailedBody", "Paperclip could not rotate the webhook secret."),
        tone: "error",
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message={t("selectCompany", "Select a company to view routines.")} />;
  }
  ...
  if (error || !routine) {
    return (
      <p className="pt-6 text-sm text-destructive">
        {error instanceof Error ? error.message : t("notFound", "Routine not found")}
      </p>
    );
  }
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="flex items-start gap-4">
        <textarea
          ...
          placeholder={t("routineTitlePlaceholder", "Routine title")}
          rows={1}
          ...
          <ToggleSwitch
            ...
            onCheckedChange={() => {
              if (!automationEnabled && !routine.assigneeAgentId) {
                pushToast({
                  title: t("toastDefaultAgentRequired", "Default agent required"),
                  body: t("toastDefaultAgentRequiredBody", "Set a default agent before enabling routine automation."),
                  tone: "warn",
                });
                return;
              }
            ...
            aria-label={automationEnabled ? t("pauseAutomaticTriggers", "Pause automatic triggers") : t("enableAutomaticTriggers", "Enable automatic triggers")}
          />
      ...
      {secretMessage && (
        <div className="rounded-lg border border-blue-500/30 bg-blue-500/5 p-4 space-y-3 text-sm">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{t("secretSaveNow", "Save this now. Paperclip will not show the secret value again.")}</p>
          </div>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Input value={secretMessage.webhookUrl} readOnly className="flex-1" />
              <Button variant="outline" size="sm" onClick={() => copySecretValue(t("webhookUrlLabel", "Webhook URL"), secretMessage.webhookUrl)}>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" size="sm" onClick={() => copySecretValue(t("webhookSecretLabel", "Webhook secret"), secretMessage.webhookSecret)}>
                <Copy className="h-3.5 w-3.5 mr-1" />
                {t("secret", "Secret")}
              </Button>
            </div>
      ...
      {!routine.assigneeAgentId ? (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4 text-sm text-amber-900 dark:text-amber-200">
          {t("defaultAgentWarning", "Default agent required. This routine can stay as a draft and still run manually, but automation stays paused until you assign a default agent.")}
        </div>
      ) : null}
      ...
      <div className="overflow-x-auto overscroll-x-contain">
        <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
          ...
          <InlineEntitySelector
            ...
            noneLabel={t("noAssignee", "No assignee")}
            searchPlaceholder={t("searchAssignees", "Search assignees...")}
            emptyMessage={t("noAssigneesFound", "No assignees found.")}
          ...
          <InlineEntitySelector
            ...
            noneLabel={t("noProjectOption", "No project")}
            ...
            renderTriggerValue={(option) =>
              option && currentProject ? (
                ...
                <span className="text-muted-foreground">{t("project", "Project")}</span>
              )
            }
            renderOption={(option) => {
              if (!option.id) return <span className="truncate">{option.label}</span>;
      ...
      <MarkdownEditor
        ...
        placeholder={t("addInstructions", "Add instructions...")}
        bordered={false}
      ...
      <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
        <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
          <span className="text-sm font-medium">{t("advancedDeliverySettings", "Advanced delivery settings")}</span>
          {advancedOpen ? <ChevronDown className="h-4 w-4 text-muted-foreground" /> : <ChevronRight className="h-4 w-4 text-muted-foreground" />}
        </CollapsibleTrigger>
        <CollapsibleContent className="pt-3">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("concurrency", "Concurrency")}</p>
              <Select
              ...
              <p className="text-xs text-muted-foreground">{t(`concurrencyDescription.${editDraft.concurrencyPolicy}`, concurrencyPolicyDescriptions[editDraft.concurrencyPolicy])}</p>
            </div>
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("catchUp", "Catch-up")}</p>
              <Select
              ...
              <p className="text-xs text-muted-foreground">{t(`catchUpDescription.${editDraft.catchUpPolicy}`, catchUpPolicyDescriptions[editDraft.catchUpPolicy])}</p>
            </div>
          </div>
        </CollapsibleContent>
      </Collapsible>
      ...
      <div className="flex items-center justify-between">
        {isEditDirty ? (
          <span className="text-xs text-amber-600">{t("unsavedChanges", "Unsaved changes")}</span>
        ) : (
          <span />
        )}
        <Button
...
<TabsTrigger value="activity" className="gap-1.5">
        ...
        <TabsContent value="triggers" className="space-y-4">
          ...
          <div className="rounded-lg border border-border p-4 space-y-3">
            <p className="text-sm font-medium">{t("addTrigger", "Add trigger")}</p>
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">{t("kind", "Kind")}</Label>
                  ...
                  <SelectContent>
                    {triggerKinds.map((kind) => (
                      <SelectItem key={kind} value={kind} disabled={kind === "webhook"}>
                        {kind}{kind === "webhook" ? t("comingSoon", " — COMING SOON") : ""}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              {newTrigger.kind === "webhook" && (
                <>
                  <div className="space-y-1.5">
                    ...
                    <p className="text-xs text-muted-foreground">{t(`signingModeDescription.${newTrigger.signingMode}`, signingModeDescriptions[newTrigger.signingMode])}</p>
                  </div>
            ...
            <div className="flex items-center justify-end">
              <Button size="sm" onClick={() => createTrigger.mutate()} disabled={createTrigger.isPending}>
                {createTrigger.isPending ? t("adding", "Adding...") : t("addTrigger", "Add trigger")}
              </Button>
            </div>
          </div>
          ...
          {routine.triggers.length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("noTriggers", "No triggers configured yet.")}</p>
          ) : (
        ...
        <TabsContent value="runs" className="space-y-4">
          ...
          {(routineRuns ?? []).length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("noRuns", "No runs yet.")}</p>
          ) : (
        ...
        <TabsContent value="activity">
          {(activity ?? []).length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("noActivity", "No activity yet.")}</p>
          ) : (
            <div className="border border-border rounded-lg divide-y divide-border">
              {(activity ?? []).map((event) => (
