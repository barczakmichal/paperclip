...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"], t: TFunction) {
  return windowKind === "lifetime" ? t("lifetimeBudget", "Lifetime budget") : t("monthlyBudget", "Monthly UTC budget");
}
...
export function BudgetPolicyCard({
  ...
  const { t } = useTranslation("budgetPolicyCard");
  ...
  const utilizationText = summary.amount > 0
    ? t("percentOfLimit", "{{percent}}% of limit", { percent: summary.utilizationPercent })
    : t("noCapConfigured", "No cap configured");
  const softAlertText = (
    <>
      {t("softAlertAt", "Soft alert at {{percent}}%", { percent: summary.warnPercent })}
      {summary.paused && summary.pauseReason ? t("pauseSuffix", " · {{reason}} pause", { reason: summary.pauseReason }) : ""}
    </>
  );
  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("observed", "Observed")}</div>
      ...
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("budget", "Budget")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("disabled", "Disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {softAlertText}
        </div>
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        ...
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : t("unlimited", "Unlimited")}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("pausedProject", "Execution is paused for this project until the budget is raised or the incident is dismissed.")
          : t("pausedScope", "Heartbeats are paused for this scope until the budget is raised or the incident is dismissed.")}
      </div>
    </div>
  ) : null;
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      ...
      >
        {isSaving ? t("saving", "Saving...") : summary.amount > 0 ? t("updateBudget", "Update budget") : t("setBudget", "Set budget")}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          ...
          >
            ...
            {summary.paused ? t("statusPaused", "Paused") : summary.status === "warning" ? t("statusWarning", "Warning") : summary.status === "hard_stop" ? t("statusHardStop", "Hard stop") : t("statusHealthy", "Healthy")}
          </div>
        </div>
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("enterValidAmount", "Enter a valid non-negative dollar amount.")}</p>
        ) : null}
      </div>
    );
  }
