...
function SidebarAgentItem({
  ...
  const { t } = useTranslation("sidebarAgents");
    ...
    <div className="group/agent relative flex items-center">
      <NavLink
        ...
        className={cn(
          ...
          isActive
            ? "bg-accent text-foreground shadow-[var(--glow-active)]"
        ...
        {(agent.pauseReason === "budget" || runCount > 0) && (
          <span className="ml-auto flex items-center gap-1.5 shrink-0">
            {agent.pauseReason === "budget" ? (
              <BudgetSidebarMarker title={t("agentPausedByBudget", "Agent paused by budget")} />
            ) : null}
            {runCount > 0 ? (
            ...
            {runCount > 0 ? (
              <span className="text-[11px] font-medium text-blue-600 dark:text-blue-400">
                {t("live", "{{count}} live", { count: runCount })}
              </span>
            ) : null}
          </span>
        )}
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            aria-label={t("openActionsFor", "Open actions for {{name}}", { name: agent.name })}
          >
        ...
        <DropdownMenuContent align="end" className="w-44">
          <DropdownMenuItem asChild>
            ...
            >
              ...
              <span>{t("editAgent", "Edit agent")}</span>
            </Link>
          </DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem
            ...
            title={isBudgetPaused ? t("agentPausedByBudgetLimits", "Agent was paused by budget limits") : undefined}
          >
...
export function SidebarAgents() {
  ...
  const pauseResumeAgent = useMutation({
    ...
    onSuccess: async (_agent, { agent, action }) => {
      ...
      pushToast({
        title: action === "pause" ? t("agentPaused", "Agent paused") : t("agentResumed", "Agent resumed"),
        body: agent.name,
        tone: "success",
      });
    },
    onError: (error, { agent, action }) => {
      pushToast({
        title: action === "pause" ? t("couldNotPauseAgent", "Could not pause agent") : t("couldNotResumeAgent", "Could not resume agent"),
      ...
      <div className="group">
        <div className="flex items-center px-3 py-1.5">
          ...
          <button
            ...
            className="flex items-center justify-center h-4 w-4 rounded text-muted-foreground/60 hover:text-foreground hover:bg-accent/40 transition-colors"
            aria-label={t("newAgent", "New agent")}
          >
