...
export function CompanySettingsSidebar() {
  const { t } = useTranslation("companySettingsSidebar");
    ...
    <aside className="w-60 h-full min-h-0 border-r border-border bg-background flex flex-col">
      <div className="flex flex-col gap-1 px-3 py-3 shrink-0">
        ...
        >
          ...
          <span className="truncate">{selectedCompany?.name ?? t("companyFallback", "Company")}</span>
        </Link>
      ...
      <nav className="flex-1 min-h-0 overflow-y-auto scrollbar-auto-hide px-3 py-2">
        <div className="flex flex-col gap-0.5">
          <SidebarNavItem to="/company/settings" label={t("general", "General")} icon={SlidersHorizontal} end />
          <SidebarNavItem
            to="/company/settings/environments"
            label={t("environments", "Environments")}
            icon={MonitorCog}
            end
          />
          <SidebarNavItem
          ...
          />
          <SidebarNavItem to="/company/settings/invites" label={t("invites", "Invites")} icon={MailPlus} end />
        </div>
      </nav>
    </aside>
  );
