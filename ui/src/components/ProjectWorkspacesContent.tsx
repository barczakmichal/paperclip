...
export function ProjectWorkspacesContent({
  ...
  const { t } = useTranslation("projectWorkspacesContent");
  ...
  if (summaries.length === 0) {
    return <p className="text-sm text-muted-foreground">{t("noActivity", "No non-default workspace activity yet.")}</p>;
  }
        ...
        {cleanupFailedSummaries.length > 0 ? (
          <div className="space-y-2">
            <div className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
              {t("cleanupAttentionNeeded", "Cleanup attention needed")}
            </div>
            <div className="space-y-3">
