  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="overflow-hidden rounded-lg border border-border/70">
      <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-2 text-left">
        <div>
          <p className="text-sm font-medium">{t("variables", "Variables")}</p>
          <p className="text-xs text-muted-foreground">
            {t("detectedFromPrefix", "Detected from `")}{"{{name}}"}{t("detectedFromSuffix", "` placeholders in the routine title and instructions.")}
          </p>
        </div>
          ...
          <div key={variable.name} className="p-4">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                {t("promptUserForValue", "Prompt the user for this value before each manual run.")}
              </span>
            </div>

            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">{t("labelLabel", "Label")}</Label>
                <Input
              ...
              <div className="space-y-1.5">
                <Label className="text-xs">{t("typeLabel", "Type")}</Label>
                <Select
                  value={variable.type}
              ...
              <div className="space-y-1.5 md:col-span-2">
                <div className="flex items-center justify-between gap-3">
                  <Label className="text-xs">{t("defaultValue", "Default value")}</Label>
                ...
                {variable.type === "textarea" ? (
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="__unset__">{t("noDefault", "No default")}</SelectItem>
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-1.5">
                      <Label className="text-xs">{t("options", "Options")}</Label>
                      <Input
                        ...
                        placeholder={t("optionsPlaceholder", "high, medium, low")}
                      />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs">{t("defaultOption", "Default option")}</Label>
                      <Select
                      ...
                      >
                        <SelectTrigger>
                          <SelectValue placeholder={t("noDefault", "No default")} />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="__unset__">{t("noDefault", "No default")}</SelectItem>
                  ...
                  <Input
                    ...
                    placeholder={variable.type === "number" ? "42" : t("defaultValuePlaceholder", "Default value")}
                  />
                )}
              </div>
            </div>
...
const BUILTIN_VARIABLE_DOCS: BuiltinVariableDoc[] = [
  {
    name: "date",
    example: "2026-04-28",
    descriptionKey: "builtinDateDesc",
    descriptionDefault: "Current date in YYYY-MM-DD format (UTC) at the time the routine runs.",
  },
  {
    name: "timestamp",
    example: "April 28, 2026 at 12:17 PM UTC",
    descriptionKey: "builtinTimestampDesc",
    descriptionDefault: "Human-readable date and time (UTC) at the time the routine runs.",
  },
];
  ...
  return (
    <>
      <div className="flex items-center justify-between gap-2 rounded-lg border border-dashed border-border/70 px-3 py-2 text-xs text-muted-foreground">
        <span>
          {t("usePlaceholdersPrefix", "Use `")}{"{{variable_name}}"}{t("usePlaceholdersSuffix", "` placeholders in the instructions to prompt for inputs when the routine runs.")}
        </span>
        <button
          type="button"
          ...
          aria-label={t("showVariableHelp", "Show variable help")}
        >
      ...
      <Dialog open={helpOpen} onOpenChange={setHelpOpen}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            <DialogTitle>{t("routineVariables", "Routine variables")}</DialogTitle>
            <DialogDescription>
              {t("dialogDescription", "How to prompt for inputs and which variables Paperclip fills in automatically.")}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-5 text-sm">
            <section className="space-y-2">
              ...
              <p className="text-muted-foreground">
                ...
                {t("typeMiddle", "anywhere in the title or instructions. Paperclip detects each placeholder, lists it under")}{" "}
                <span className="font-medium text-foreground">{t("variables", "Variables")}</span>
                {t("typeSuffix", ", and prompts for a value before each run.")}
              </p>
              <ul className="list-disc space-y-1 pl-5 text-muted-foreground">
                <li>{t("ruleNames", "Names must start with a letter and may use letters, numbers, and underscores.")}</li>
                <li>{t("rulePickType", "Pick a type (text, textarea, number, boolean, select), default value, and whether it is required.")}</li>
                <li>{t("ruleReuse", "The same name reused across the title and instructions is treated as one variable.")}</li>
              </ul>
            </section>
            ...
            <section className="space-y-2">
              ...
              <p className="text-muted-foreground">
                {t("builtinDescription", "These are filled in automatically — no setup needed and they will not appear in the Variables list.")}
              </p>
              <div className="overflow-hidden rounded-lg border border-border/70">
                <table className="w-full text-left text-xs">
                  <thead className="bg-muted/40 text-muted-foreground">
                    <tr>
                      <th className="px-3 py-2 font-medium">{t("colPlaceholder", "Placeholder")}</th>
                      <th className="px-3 py-2 font-medium">{t("colExample", "Example")}</th>
                      <th className="px-3 py-2 font-medium">{t("colDescription", "Description")}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border/70">
                    {BUILTIN_VARIABLE_DOCS.map((entry) => (
                      <tr key={entry.name} className="align-top">
                        ...
                        <td className="px-3 py-2 text-muted-foreground">{t(entry.descriptionKey, entry.descriptionDefault)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
