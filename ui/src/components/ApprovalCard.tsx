...
export function ApprovalCard({
  ...
  const { t } = useTranslation("approvalCard");
  ...
  return (
    <GlowFrame state={glowState(approval.status)} className="rounded-xl bg-card p-4 shadow-sm">
      <div className="flex items-start justify-between gap-4">
            ...
            <div className="min-w-0 flex-1 space-y-2">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs leading-5 text-muted-foreground">
                  {t("createdAgo", "Approval request created {{time}}", { time: timeAgo(approval.createdAt) })}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="shrink-0">
          {approval.status === "pending" ? (
            <LiveDot status="warning" pulse label={t("awaitingApproval", "awaiting approval")} />
          ) : (
      ...
      {approval.decisionNote && (
        <div className="mt-4 rounded-lg border border-border/60 bg-muted/30 px-3.5 py-3 text-xs leading-5 text-muted-foreground">
          <span className="font-medium text-foreground">{t("decisionNote", "Decision note.")}</span> {approval.decisionNote}
        </div>
      )}
            ...
            {showResolutionButtons && (
              <>
                <Button
                  ...
                  disabled={isPending}
                >
                  {pendingAction === "approve" ? t("approving", "Approving...") : t("approve", "Approve")}
                </Button>
                <Button
                  variant="destructive"
                  size="sm"
                  ...
                  disabled={isPending}
                >
                  {pendingAction === "reject" ? t("rejecting", "Rejecting...") : t("reject", "Reject")}
                </Button>
              </>
            )}
          </div>
