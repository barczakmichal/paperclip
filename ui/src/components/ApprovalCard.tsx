    ...
    <div className="rounded-xl border border-border/70 bg-card p-4 shadow-sm">
            ...
            <div className="min-w-0 flex-1 space-y-2">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs leading-5 text-muted-foreground">
                  {t("approval.approvalRequestCreated", { timeAgo: timeAgo(approval.createdAt) })}
                </p>
              </div>
            </div>
          </div>
      ...
      {approval.decisionNote && (
        <div className="mt-4 rounded-lg border border-border/60 bg-muted/30 px-3.5 py-3 text-xs leading-5 text-muted-foreground">
          <span className="font-medium text-foreground">{t("approval.decisionNote")}.</span> {approval.decisionNote}
        </div>
      )}
            ...
            {showResolutionButtons && (
              <>
                <Button
                  ...
                  disabled={isPending}
                >
                  {pendingAction === "approve" ? t("approval.approving") : t("approval.approve")}
                </Button>
                <Button
                  variant="destructive"
                  size="sm"
                  ...
                  disabled={isPending}
                >
                  {pendingAction === "reject" ? t("approval.rejecting") : t("approval.reject")}
                </Button>
              </>
            )}
          </div>
