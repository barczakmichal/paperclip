...
function IssueAssigneePausedNotice({ agent, t }: { agent: Agent | null; t: (key: string, options?: Record<string, unknown>) => string }) {
  if (!agent || agent.status !== "paused") return null;

  const pauseDetail =
    agent.pauseReason === "budget"
      ? t("issueChat.pauseReason.budget")
      : agent.pauseReason === "system"
        ? t("issueChat.pauseReason.system")
        : t("issueChat.pauseReason.manual");
    ...
    <div className="mb-3 rounded-md border border-orange-300/70 bg-orange-50/90 px-3 py-2.5 text-sm text-orange-950 shadow-sm dark:border-orange-500/40 dark:bg-orange-500/10 dark:text-orange-100">
      <div className="flex items-start gap-2">
        ...
        <p className="min-w-0 leading-5">
          {t("issueChat.agentPaused", { name: agent.name, reason: pauseDetail })}
        </p>
      </div>
    </div>
  );
...
function fallbackAuthorLabel(message: ThreadMessage, t: (key: string) => string) {
  ...
  if (message.role === "assistant") return t("issueChat.actor.agent");
  if (message.role === "user") return t("issueChat.actor.you");
  return t("issueChat.actor.system");
}
      ...
      <div className="rounded-xl border border-amber-300/60 bg-amber-50/80 px-4 py-3 text-sm text-amber-900 dark:border-amber-500/30 dark:bg-amber-950/20 dark:text-amber-200">
        <div className="flex items-start gap-2">
          ...
          <div className="space-y-1">
            <p className="font-medium">{t("issueChat.fallback.errorTitle")}</p>
              ...
              <div key={message.id} className="rounded-xl border border-border/60 bg-card/70 px-4 py-3">
                ...
                <div className="space-y-2">
                  {lines.length > 0 ? lines.map((line, index) => (
                    ...
                    <p className="text-sm text-muted-foreground">{t("issueChat.fallback.noContent")}</p>
                  )}
                </div>
              </div>
            );
...
function humanizeValue(value: string | null, t: (key: string) => string) {
  if (!value) return t("issueChat.actor.none");
...
function formatInteractionActorLabel(args: {
  ...
  if (userId) {
    return userLabelMap?.get(userId)
      ...
      ?? translate("issueChat.actor.board");
  }
  return translate("issueChat.actor.system");
}
...
export function resolveIssueChatHumanAuthor(args: {
  ...
  const resolvedAuthorName = profile?.label?.trim()
    || authorName?.trim()
    || (authorUserId === "local-board" ? translate("issueChat.actor.board") : (isCurrentUser ? translate("issueChat.actor.you") : translate("issueChat.actor.user")));
...
function toolCountSummary(toolParts: ToolCallMessagePart[], t: (key: string, options?: Record<string, unknown>) => string): string | null {
  ...
  const parts: string[] = [];
  if (commands > 0) parts.push(t("component.issueChatThread.toolSummaryCommand", { count: commands }));
  if (other > 0) parts.push(t("component.issueChatThread.toolSummaryTool", { count: other }));
  return parts.join(", ");
}
...
function IssueChatChainOfThought({
  ...
  if (isActive) {
    headerVerb = t("component.issueChatThread.workingHeader");
    if (liveElapsed) headerSuffix = t("component.issueChatThread.headerFor", { duration: liveElapsed });
  } else if (segmentTiming) {
    const durationMs = segmentTiming.endMs - segmentTiming.startMs;
    const durationText = formatDurationWords(durationMs);
    headerVerb = t("component.issueChatThread.workedHeader");
    if (durationText) headerSuffix = t("component.issueChatThread.headerFor", { duration: durationText });
  } else {
    headerVerb = t("component.issueChatThread.workedHeader");
  }
  ...
  return (
    <div className="group/pre relative">
      ...
      <button
        ...
        title={t("component.issueChatThread.copy")}
        aria-label={t("component.issueChatThread.copy")}
        onClick={() => {
        ...
        {open ? (
          <div className="mt-1 space-y-2 pb-1">
            {nonIntentDetails.length > 0 ? (
              <div>
                <div className="mb-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground/60">
                  {t("component.issueChatThread.input")}
                </div>
            ...
            {result !== undefined ? (
              <div>
                <div className="mb-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground/60">
                  {t("component.issueChatThread.result")}
                </div>
...
function IssueChatUserMessage({
  ...
  const queueBadgeLabel = queueReason === "hold" ? t("issueChat.queue.deferred") : t("issueChat.queue.queued");
    ...
    <div className={cn("flex min-w-0 max-w-[85%] flex-col", isCurrentUser && "items-end")}>
      <div className={cn("mb-1 flex items-center gap-2 px-1", isCurrentUser ? "justify-end" : "justify-start")}>
        ...
        {followUpRequested ? (
          <Badge variant="outline" className="text-[10px] uppercase tracking-[0.14em]">
            {t("component.issueChatThread.follow_up")}
          </Badge>
        ) : null}
      </div>
      <div
        ...
        {queued ? (
          <div className="mb-1.5 flex items-center gap-2">
            ...
            {queueTargetRunId && onInterruptQueued ? (
              ...
              >
                {isInterruptingQueuedRun ? t("issueChat.queue.interrupting") : t("issueChat.queue.interrupt")}
              </Button>
            ) : null}
            {onCancelQueued ? (
              <Button
              ...
              >
                {t("component.issueChatThread.cancel")}
              </Button>
            ) : null}
          </div>
        ) : null}
      ...
      {pending ? (
        <div className={cn("mt-1 flex px-1 text-[11px] text-muted-foreground", isCurrentUser ? "justify-end" : "justify-start")}>
          {t("component.issueChatThread.sending")}
        </div>
      ) : (
        <div
          className={cn(
        ...
        >
          ...
          {!deleted ? (
            <button
              ...
              title={t("component.issueChatThread.copyMessage")}
              aria-label={t("component.issueChatThread.copyMessage")}
              onClick={() => {
...
function IssueChatAssistantMessage({
  ...
  const resolvedStopRunLabel = stopRunLabel ?? t("issueChat.run.stop");
  const resolvedStoppingRunLabel = stoppingRunLabel ?? t("issueChat.run.stopping");
  const custom = message.metadata.custom as Record<string, unknown>;
  const anchorId = typeof custom.anchorId === "string" ? custom.anchorId : undefined;
  const authorName = typeof custom.authorName === "string"
    ...
    : typeof custom.runAgentName === "string"
      ? custom.runAgentName
      : t("component.issueChatThread.agentFallback");
  const authorAgentId = typeof custom.authorAgentId === "string" ? custom.authorAgentId : null;
            ...
            <div className="mb-1.5 flex items-center gap-2">
              ...
              {isRunning ? (
                <span className="inline-flex items-center gap-1 rounded-full border border-cyan-400/40 bg-cyan-500/10 px-2 py-0.5 text-[10px] font-medium uppercase tracking-[0.14em] text-cyan-700 dark:text-cyan-200">
                  ...
                  {t("component.issueChatThread.running")}
                </span>
              ) : null}
            </div>
          )}
  ...
  return (
    <>
      <button
        ...
        title={t("component.issueChatThread.helpful")}
        aria-label={t("component.issueChatThread.helpful")}
      ...
      <Popover open={reasonOpen} onOpenChange={setReasonOpen}>
        <PopoverTrigger asChild>
          <button
            ...
            title={t("component.issueChatThread.needsWork")}
            aria-label={t("component.issueChatThread.needsWork")}
        ...
        <PopoverContent side="top" align="start" className="w-80 p-3">
          <div className="mb-2 text-sm font-medium">{t("component.issueChatThread.feedbackPrompt")}</div>
          <Textarea
            value={downvoteReason}
            onChange={(event) => setDownvoteReason(event.target.value)}
            placeholder={t("component.issueChatThread.addShortNote")}
          ...
          <div className="mt-2 flex items-center justify-end gap-2">
            ...
            >
              {t("component.issueChatThread.dismiss")}
            </Button>
            <Button
              type="button"
              size="sm"
            ...
            >
              {isSaving ? t("component.issueChatThread.saving") : t("component.issueChatThread.saveNote")}
            </Button>
          </div>
        </PopoverContent>
      </Popover>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("component.issueChatThread.feedbackDialog.title")}</DialogTitle>
            <DialogDescription>
              {t("component.issueChatThread.feedbackDialog.description")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm text-muted-foreground">
            <p>{t("component.issueChatThread.feedbackDialog.localOnly")}</p>
            <p>
              <Trans
                i18nKey="component.issueChatThread.feedbackDialog.instruction"
                components={[
            ...
            <p>{t("component.issueChatThread.feedbackDialog.changeLater")}</p>
            {termsUrl ? (
              <a
                href={termsUrl}
                target="_blank"
              ...
              >
                {t("component.issueChatThread.feedbackDialog.termsLink")}
              </a>
            ) : null}
          </div>
          <DialogFooter>
            ...
            >
              {isSaving ? t("component.issueChatThread.saving") : t("component.issueChatThread.dontAllow")}
            </Button>
            <Button
              type="button"
              disabled={!pendingSharingDialog || isSaving}
            ...
            >
              {isSaving ? t("component.issueChatThread.saving") : t("component.issueChatThread.alwaysAllow")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
  ...
  const rowContent = (
    <div className="min-w-0 flex-1">
      <div className={cn("flex flex-wrap items-center gap-x-1.5 gap-y-1 text-xs", isCurrentUser && "justify-end")}>
        ...
        <span className="text-muted-foreground">{t("component.issueChatThread.updatedThisTask")}</span>
        <a
          href={anchorId ? `#${anchorId}` : undefined}
        ...
        >
          ...
          {expanded ? t("component.issueChatThread.hideConfirmation") : t("component.issueChatThread.expiredConfirmation")}
        </button>
      </div>
      {expanded ? (
...
function StaleDispositionWarningDetails({
  ...
  if (sections.length === 0) {
    return <div className="text-xs leading-5 text-muted-foreground">{t("component.issueChatThread.noAdditionalDetails")}</div>;
  }
        ...
        <div className="min-w-0 flex-1">
          ...
          >
            <span className="text-sm font-medium text-foreground/80">
              {t("component.issueChatThread.staleDispositionWarning")}
            </span>
            <span className="ml-auto flex items-center gap-1.5">
              {message.createdAt ? (
...
function SystemNoticeCommentRow({
  ...
  const source = (() => {
    ...
    if (authorType === "system") {
      const label = runAgentName ?? t("component.issueChatThread.paperclipFallback");
      if (runAgentId && runId) return { label, href: `/agents/${runAgentId}/runs/${runId}` };
      return { label };
    }
    if (runAgentId && runId) {
      return { label: authorName ?? runAgentName ?? t("component.issueChatThread.paperclipFallback"), href: `/agents/${runAgentId}/runs/${runId}` };
    }
        ...
        <div className="mt-1 flex items-center justify-end gap-1.5 px-1 opacity-0 transition-opacity group-hover:opacity-100">
          ...
          {anchorId ? (
            <button
              type="button"
              className="inline-flex h-6 w-6 items-center justify-center text-muted-foreground transition-colors hover:text-foreground"
              title={t("component.issueChatThread.copyLink")}
              aria-label={t("component.issueChatThread.copyLinkToSystemNotice")}
          ...
          <button
            type="button"
            className="inline-flex h-6 w-6 items-center justify-center text-muted-foreground transition-colors hover:text-foreground"
            title={t("component.issueChatThread.copyNoticeText")}
            aria-label={t("component.issueChatThread.copySystemNotice")}
            onClick={handleCopy}
          >
            {copied ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}
          </button>
      ...
      <IssueChatMetadataRow anchorId={anchorId} icon={rowIcon}>
        <div className="flex flex-wrap items-baseline gap-x-1.5 gap-y-0.5 text-xs">
          ...
          <span className="text-muted-foreground">
            {custom.followUpRequested === true ? t("component.issueChatThread.requestedFollowUp") : t("component.issueChatThread.updatedThisTask")}
          </span>
          <a
            href={anchorId ? `#${anchorId}` : undefined}
        ...
        {statusChange ? (
          <div className="flex flex-wrap items-center gap-1.5 text-xs">
            <span className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground/70">
              {t("component.issueChatThread.statusLabel")}
            </span>
        ...
        {assigneeChange ? (
          <div className="space-y-1">
            <div className={cn("flex flex-wrap items-center gap-1.5 text-xs", isCurrentUser && "justify-end")}>
              <span className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground/70">
                {t("component.issueChatThread.assigneeLabel")}
              </span>
        ...
        {workspaceChange ? (
          <div className="flex flex-wrap items-center gap-1.5 text-xs">
            <span className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground/70">
              {t("component.issueChatThread.workspaceLabel")}
            </span>
            <span className="text-muted-foreground">
    ...
    return (
      <IssueChatMetadataRow anchorId={anchorId} icon={rowIcon}>
        <div className="flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-xs">
          ...
          <span className="text-muted-foreground">{t("component.issueChatThread.runLabel")}</span>
          <Link
  ...
  async function attachFile(file: File) {
    ...
    try {
      if (onImageUpload && file.type.startsWith("image/")) {
        ...
        setComposerAttachments((prev) => prev.map((item) =>
          item.id === attachmentId
            ? { ...item, status: "error", error: t("component.issueChatThread.composer.cannotAttachFileType") }
            : item,
        ));
      }
    } catch (err) {
      setComposerAttachments((prev) => prev.map((item) =>
        item.id === attachmentId
          ? {
              ...item,
              status: "error",
              error: err instanceof Error ? err.message : t("component.issueChatThread.composer.uploadFailed"),
            }
          : item,
      ));
    }
  ...
  return (
    ...
    >
        ...
        >
          <div className="flex max-w-md items-center gap-3 rounded-md bg-background/80 px-3 py-2 text-left shadow-sm ring-1 ring-border/60">
            ...
            <div className="min-w-0">
              <div className="text-sm font-medium text-foreground">{t("component.issueChatThread.composer.dropToUpload")}</div>
      ...
      <MarkdownEditor
        ...
        value={body}
        onChange={setBody}
        placeholder={t("component.issueChatThread.composer.reply")}
        mentions={mentions}
            ...
            const statusLabel =
              attachment.status === "uploading"
                ...
                : attachment.status === "error"
                  ? attachment.error ?? t("component.issueChatThread.composer.uploadFailed")
                  : attachment.inline
                    ? t("component.issueChatThread.composer.insertedInline")
                    : "Attached to task";
            return (
              <div
                key={attachment.id}
      ...
      <div className="flex flex-wrap items-center justify-end gap-3">
          ...
          {(onImageUpload || onAttachImage) ? (
            <>
              ...
              <Button
                ...
                disabled={attaching}
                title={t("component.issueChatThread.composer.attachFile")}
              >
                <Paperclip className="h-4 w-4" />
              </Button>
            </>
        ...
        {enableReassign && reassignOptions.length > 0 ? (
          <InlineEntitySelector
            ...
            placeholder={t("component.issueChatThread.composer.assignee")}
            noneLabel={t("component.issueChatThread.composer.noAssignee")}
            searchPlaceholder={t("component.issueChatThread.composer.searchAssignees")}
            emptyMessage={t("component.issueChatThread.composer.noAssigneesFound")}
            onChange={setReassignTarget}
            className="h-8 text-xs"
            renderTriggerValue={(option) => {
              if (!option) return <span className="text-muted-foreground">{t("component.issueChatThread.composer.assignee")}</span>;
        ...
        <Button size="sm" disabled={!canSubmit} onClick={() => void handleSubmit()}>
          {submitting ? t("component.issueChatThread.composer.posting") : t("component.issueChatThread.composer.send")}
        </Button>
      </div>
...
export function IssueChatThread({
  ...
  const resolvedEmptyMessage = emptyMessage
    ?? (variant === "embedded"
      ? t("component.issueChatThread.noRunOutputYet")
      ...
      <div className={cn(variant === "embedded" ? "space-y-3" : "space-y-4")}>
        {resolvedShowJumpToLatest ? (
          <div className="flex justify-end">
            ...
            >
              {t("component.issueChatThread.jumpToLatest")}
            </button>
          </div>
        ) : null}
            ...
            >
              ...
              {showComposer ? (
                <div data-testid="issue-chat-thread-notices" className="space-y-2">
                  ...
                  />
                  <IssueAssigneePausedNotice agent={assignedAgent} t={t} />
                </div>
              ) : null}
