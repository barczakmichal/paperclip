...
export function LiveOpsTopMetrics({ agents, pendingApprovals, tasksDoneToday }: LiveOpsTopMetricsProps) {
  ...
  return (
    <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
      <MetricCard icon={Bot} value={activeCount} label="Active agents" />
      <MetricCard icon={CheckCircle2} value={tasksDoneToday} label="Tasks done today" />
      <MetricCard icon={DollarSign} value={`$${(costTodayCents / 100).toFixed(2)}`} label="Cost today" />
      <MetricCard icon={Clock} value={pendingApprovals} label="Pending approvals" />
    </div>
  );
}
