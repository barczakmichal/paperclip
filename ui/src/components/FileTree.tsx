...
export function FileTree({
  ...
  const effectiveAriaLabel = ariaLabel ?? t("pages.fileTree.files");
    ...
    return (
      <div aria-label={effectiveAriaLabel} role="tree" className="p-3">
        <div className="rounded-md border border-dashed border-border px-4 py-8 text-center">
          <div className="text-sm font-medium">{empty?.title ?? t("pages.fileTree.noFiles")}</div>
          <div className="mt-1 text-xs text-muted-foreground">
            {empty?.description ?? t("pages.fileTree.emptyDescription")}
          </div>
        </div>
      </div>
    );
          ...
          >
            ...
            {node.kind === "dir" && (
              <button
                ...
                aria-label={expanded ? t("pages.fileTree.collapse", { name: node.name }) : t("pages.fileTree.expand", { name: node.name })}
              >
                {expanded ? (
