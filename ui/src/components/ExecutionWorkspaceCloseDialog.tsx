...
export function ExecutionWorkspaceCloseDialog({
  ...
  const actionLabel = currentStatus === "cleanup_failed" ? t("pages.executionWorkspaceClose.retryClose") : t("pages.executionWorkspaceClose.closeWorkspace");
  ...
  const closeWorkspace = useMutation({
    ...
    onSuccess: (workspace) => {
      ...
      pushToast({
        title: currentStatus === "cleanup_failed" ? t("pages.executionWorkspaceClose.workspaceCloseRetried") : t("pages.executionWorkspaceClose.workspaceClosed"),
        tone: "success",
      });
      onOpenChange(false);
      onClosed?.(workspace);
    ...
    onError: (error) => {
      pushToast({
        title: t("pages.executionWorkspaceClose.failedToCloseWorkspace"),
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        ...
        {readinessQuery.isLoading ? (
          <div className="flex items-center gap-2 rounded-xl border border-border bg-background px-4 py-3 text-sm text-muted-foreground">
            ...
            {t("pages.executionWorkspaceClose.checkingSafety")}
          </div>
        ) : readinessQuery.error ? (
          <div className="rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive">
            {readinessQuery.error instanceof Error ? readinessQuery.error.message : t("pages.executionWorkspaceClose.failedToInspect")}
          </div>
        ) : readiness ? (
          <div className="space-y-4">
            <div className={`rounded-xl border px-4 py-3 text-sm ${readinessTone(readiness.state)}`}>
              <div className="font-medium">
                {readiness.state === "blocked"
                  ? t("pages.executionWorkspaceClose.closeBlocked")
                  : readiness.state === "ready_with_warnings"
                    ? t("pages.executionWorkspaceClose.closeAllowedWithWarnings")
                    : t("pages.executionWorkspaceClose.closeReady")}
              </div>
              <div className="mt-1 text-xs opacity-80">
                {readiness.isSharedWorkspace
                  ? t("pages.executionWorkspaceClose.sharedWorkspaceArchiving")
                  : readiness.git?.workspacePath && readiness.git.repoRoot && readiness.git.workspacePath !== readiness.git.repoRoot
                    ? t("pages.executionWorkspaceClose.ownCheckoutPath")
                    : readiness.isProjectPrimaryWorkspace
                      ? t("pages.executionWorkspaceClose.primaryWorkspacePath")
                      : t("pages.executionWorkspaceClose.disposableWorkspace")}
              </div>
            </div>
            ...
            {readiness.blockingReasons.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("pages.executionWorkspaceClose.blockingReasons")}</h3>
            ...
            {readiness.warnings.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("pages.executionWorkspaceClose.warnings")}</h3>
                <ul className="space-y-2 text-sm text-muted-foreground">
            ...
            {readiness.git ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("pages.executionWorkspaceClose.gitStatus")}</h3>
                <div className="rounded-xl border border-border bg-background px-4 py-3 text-sm">
                  <div className="grid gap-2 sm:grid-cols-2">
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("pages.executionWorkspaceClose.branch")}</div>
                      <div className="font-mono text-xs">{readiness.git.branchName ?? t("pages.executionWorkspaceClose.unknown")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("pages.executionWorkspaceClose.baseRef")}</div>
                      <div className="font-mono text-xs">{readiness.git.baseRef ?? t("pages.executionWorkspaceClose.notSet")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("pages.executionWorkspaceClose.mergedIntoBase")}</div>
                      <div>{readiness.git.isMergedIntoBase == null ? t("pages.executionWorkspaceClose.unknown") : readiness.git.isMergedIntoBase ? t("pages.executionWorkspaceClose.yes") : t("pages.executionWorkspaceClose.no")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("pages.executionWorkspaceClose.aheadBehind")}</div>
                      <div>
                        {(readiness.git.aheadCount ?? 0).toString()} / {(readiness.git.behindCount ?? 0).toString()}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("pages.executionWorkspaceClose.dirtyTrackedFiles")}</div>
                      <div>{readiness.git.dirtyEntryCount}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("pages.executionWorkspaceClose.untrackedFiles")}</div>
            ...
            {readiness.runtimeServices.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("pages.executionWorkspaceClose.attachedRuntimeServices")}</h3>
                <div className="space-y-2">
            ...
            <section className="space-y-2">
              <h3 className="text-sm font-medium">{t("pages.executionWorkspaceClose.cleanupActions")}</h3>
              <div className="space-y-2">
            ...
            {currentStatus === "cleanup_failed" ? (
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-sm text-muted-foreground">
                {t("pages.executionWorkspaceClose.cleanupFailedRetry")}
              </div>
            ) : null}

            {currentStatus === "archived" ? (
              <div className="rounded-xl border border-border bg-background px-4 py-3 text-sm text-muted-foreground">
                {t("pages.executionWorkspaceClose.alreadyArchived")}
              </div>
            ) : null}

            {readiness.git?.repoRoot ? (
              <div className="break-words text-xs text-muted-foreground">
                {t("pages.executionWorkspaceClose.repoRoot")}{" "}
                <span className="font-mono break-all">{readiness.git.repoRoot}</span>
                {readiness.git.workspacePath ? (
                  <>
                    {" · "}{t("pages.executionWorkspaceClose.workspacePath")}{" "}
                    <span className="font-mono break-all">{readiness.git.workspacePath}</span>
                  </>
                ) : null}
              </div>
            ) : null}
            ...
            <div className="text-xs text-muted-foreground">
              {t("pages.executionWorkspaceClose.lastChecked", { time: formatDateTime(new Date()) })}
            </div>
          </div>
        ) : null}
        ...
        <DialogFooter>
          ...
          >
            {t("pages.executionWorkspaceClose.cancel")}
          </Button>
          <Button
