...
function SkillList({ values }: { values: unknown }) {
  const { t } = useTranslation("approvalPayload");
  ...
  return (
    <div className="flex items-start gap-2">
      <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs pt-0.5">{t("skills", "Skills")}</span>
...
export function HireAgentPayload({ payload }: { payload: Record<string, unknown> }) {
  ...
  return (
    <div className="mt-3 space-y-1.5 text-xs text-foreground bg-muted/30 border border-border rounded-md p-3">
      <div className="flex items-center gap-2">
        <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs">{t("name", "Name")}</span>
        <span className="font-medium">{String(payload.name ?? "—")}</span>
      </div>
      <PayloadField label={t("role", "Role")} value={payload.role} />
      <PayloadField label={t("title", "Title")} value={payload.title} />
      <PayloadField label={t("icon", "Icon")} value={payload.icon} />
      {!!payload.capabilities && (
        <div className="flex items-start gap-2">
          <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs pt-0.5">{t("capabilities", "Capabilities")}</span>
          <span className="text-muted-foreground">{String(payload.capabilities)}</span>
        </div>
      )}
      {!!payload.adapterType && (
        <div className="flex items-center gap-2">
          <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs">{t("adapter", "Adapter")}</span>
...
export function BudgetOverridePayload({ payload }: { payload: Record<string, unknown> }) {
  ...
  return (
    <div className="mt-3 space-y-1.5 text-xs text-foreground bg-muted/30 border border-border rounded-md p-3">
      <PayloadField label={t("scope", "Scope")} value={payload.scopeName ?? payload.scopeType} />
      <PayloadField label={t("window", "Window")} value={payload.windowKind} />
      <PayloadField label={t("metric", "Metric")} value={payload.metric} />
      {(budgetAmount !== null || observedAmount !== null) ? (
        <div className="rounded-md bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
          {t("limitObserved", "Limit {{limit}} · Observed {{observed}}", {
    ...
    <div className="mt-4 space-y-3.5 text-sm">
      {title && (
        <div className="space-y-1">
          <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">{t("title", "Title")}</p>
          <p className="font-medium leading-6 text-foreground">{title}</p>
        </div>
      )}
      {summary && (
        <div className="space-y-1">
          <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">{t("summary", "Summary")}</p>
      ...
      {nextActionOnApproval && (
        <div className="rounded-lg border border-border/60 bg-background/60 px-3.5 py-3">
          <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">{t("onApproval", "On approval")}</p>
          <p className="mt-1 leading-6 text-foreground">{nextActionOnApproval}</p>
        </div>
      )}
      {risks.length > 0 && (
        <div className="space-y-1.5">
          <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">{t("risks", "Risks")}</p>
      ...
      {proposedComment && (
        <div className="space-y-1.5">
          <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">
            {t("proposedComment", "Proposed comment")}
          </p>
