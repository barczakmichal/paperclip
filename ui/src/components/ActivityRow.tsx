...
export function ActivityRow({ event, agentMap, userProfileMap, entityNameMap, entityTitleMap, className }: ActivityRowProps) {
  const { t } = useTranslation("activityRow");
  ...
  const actorName = actor?.name ?? (event.actorType === "system" ? t("system", "System") : userProfile?.label ?? (event.actorType === "user" ? t("board", "Board") : event.actorId || t("unknown", "Unknown")));
  ...
  const classes = cn(
    ...
    link && "cursor-pointer hover:bg-accent/30",
    className,
  );
