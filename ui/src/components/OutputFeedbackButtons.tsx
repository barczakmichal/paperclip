...
export function OutputFeedbackButtons({
  ...
  const { t } = useTranslation("outputFeedbackButtons");
  ...
  return (
    <>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("dialogTitle", "Save your feedback sharing preference")}</DialogTitle>
            <DialogDescription>
              {t("dialogDescription", "Choose whether voted AI outputs can be shared with Paperclip Labs. This answer becomes the default for future thumbs up and thumbs down votes.")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm text-muted-foreground">
            <p>
              {t("voteSavedLocally", "This vote is always saved locally.")}
            </p>
            <p>
              <Trans
                i18nKey="chooseSharing"
                t={t}
                defaults="Choose <0>Always allow</0> to share this vote and future voted AI outputs. Choose <1>Don't allow</1> to keep this vote and future votes local."
                components={[
            ...
            <p>
              {t("changeLater", "You can change this later in Instance Settings > General.")}
            </p>
            {termsUrl ? (
              <a
                href={termsUrl}
