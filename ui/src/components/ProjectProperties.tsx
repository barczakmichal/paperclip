  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {currentStatus ? t(currentStatus.labelKey) : status.replace("_", " ")}
        </button>
      </PopoverTrigger>
...
function ArchiveDangerZone({
  ...
  const action = isArchive ? t("common.actions.archive") : t("common.actions.unarchive");

  return (
    <div className="space-y-3 rounded-md border border-destructive/40 bg-destructive/5 px-4 py-4">
      <p className="text-sm text-muted-foreground">
        {isArchive
          ? t("component.properties.archiveDescription")
          : t("component.properties.unarchiveDescription")}
      </p>
      {archivePending ? (
        <Button size="sm" variant="destructive" disabled>
          <Loader2 className="h-3 w-3 animate-spin mr-1" />
          {isArchive ? t("component.properties.archiving") : t("component.properties.unarchiving")}
        </Button>
      ) : confirming ? (
...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
  ...
  const submitLocalWorkspace = () => {
    ...
    if (!isAbsolutePath(cwd)) {
      setWorkspaceError(t("component.properties.localFolderMustBeAbsolutePath"));
      return;
    }
    setWorkspaceError(null);
    persistCodebase({ cwd });
  ...
  const submitRepoWorkspace = () => {
    ...
    if (!looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(t("component.properties.repoMustBeValidGitHubUrl"));
      return;
    }
    setWorkspaceError(null);
  ...
  const clearLocalWorkspace = () => {
    const confirmed = window.confirm(
      codebase.repoUrl
        ? t("component.properties.clearLocalFolderFromWorkspace")
        : t("component.properties.deleteLocalFolder"),
    );
    if (!confirmed) return;
    persistCodebase({ cwd: null });
  };
  ...
  const clearRepoWorkspace = () => {
    ...
    const confirmed = window.confirm(
      hasLocalFolder
        ? t("component.properties.clearRepoFromWorkspace")
        : t("component.properties.deleteWorkspaceRepo"),
    );
    if (!confirmed) return;
  ...
  return (
    <div>
      <div className="space-y-1 pb-4">
        <PropertyRow label={<FieldLabel label={t("common.form.name")} state={fieldState("name")} />}>
          {onUpdate || onFieldUpdate ? (
            <DraftInput
              ...
              placeholder={t("component.properties.projectNamePlaceholder")}
            />
          ) : (
        ...
        <PropertyRow
          label={<FieldLabel label={t("common.form.description")} state={fieldState("description")} />}
          alignStart
        ...
        >
          {onUpdate || onFieldUpdate ? (
            <InlineEditor
              ...
              placeholder={t("component.properties.addDescriptionPlaceholder")}
              multiline
            />
          ) : (
            <p className="text-sm text-muted-foreground">
              {project.description?.trim() || t("component.properties.noDescription")}
            </p>
          )}
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("common.form.status")} state={fieldState("status")} />}>
          {onUpdate || onFieldUpdate ? (
            <ProjectStatusPicker
        ...
        {project.leadAgentId && (
          <PropertyRow label={t("component.properties.lead")}>
            <span className="text-sm font-mono">{project.leadAgentId.slice(0, 8)}</span>
          </PropertyRow>
        )}
        <PropertyRow
          label={<FieldLabel label={t("common.form.goal")} state={fieldState("goals")} />}
          alignStart
            ...
            <Popover open={goalOpen} onOpenChange={setGoalOpen}>
              <PopoverTrigger asChild>
                ...
                >
                  ...
                  {t("component.properties.goal")}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-56 p-1" align="start">
                {availableGoals.length === 0 ? (
                  <div className="px-2 py-1.5 text-xs text-muted-foreground">
                    {t("component.properties.allGoalsLinked")}
                  </div>
                ) : (
        ...
        <PropertyRow
          label={<FieldLabel label={t("component.properties.env")} state={fieldState("env")} />}
          alignStart
          valueClassName="space-y-2"
        >
        ...
        <PropertyRow label={<FieldLabel label={t("common.form.created")} state="idle" />}>
          <span className="text-sm">{formatDate(project.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("common.form.updated")} state="idle" />}>
          <span className="text-sm">{formatDate(project.updatedAt)}</span>
        </PropertyRow>
        {project.targetDate && (
          <PropertyRow label={<FieldLabel label={t("component.properties.targetDate")} state="idle" />}>
      ...
      <div className="space-y-1 py-4">
        <div className="space-y-2">
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <span>{t("component.properties.codebase")}</span>
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  type="button"
                  className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-border text-[10px] text-muted-foreground hover:text-foreground"
                  aria-label={t("component.properties.codebaseHelp")}
                >
                  ?
                </button>
              </TooltipTrigger>
              <TooltipContent side="top">
                {t("component.properties.codebaseHelpTooltip")}
              </TooltipContent>
            </Tooltip>
          </div>
          <div className="space-y-2 rounded-md border border-border/70 p-3">
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("component.properties.repo")}</div>
              {codebase.repoUrl ? (
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    >
                      {t("component.properties.changeRepo")}
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon-xs"
                      onClick={clearRepoWorkspace}
                      aria-label={t("component.properties.clearRepo")}
                    >
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="text-xs text-muted-foreground">{t("component.properties.notSet")}</div>
                  <Button
                    variant="outline"
                    size="xs"
                    className="h-6 px-2"
                  ...
                  >
                    {t("component.properties.setRepo")}
                  </Button>
                </div>
              )}
            </div>
            ...
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("component.properties.localFolder")}</div>
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0 space-y-1">
                  ...
                  {codebase.origin === "managed_checkout" && (
                    <div className="text-[11px] text-muted-foreground">{t("component.properties.paperclipManagedFolder")}</div>
                  )}
                </div>
                <div className="flex items-center gap-1">
                  <Button
                  ...
                  >
                    {codebase.localFolder ? t("component.properties.changeLocalFolder") : t("component.properties.setLocalFolder")}
                  </Button>
                  {codebase.localFolder ? (
                    <Button
                      ...
                      aria-label={t("component.properties.clearLocalFolder")}
                    >
                      <Trash2 className="h-3 w-3" />
                    </Button>
                  ) : null}
            ...
            {hasAdditionalLegacyWorkspaces && (
              <div className="text-[11px] text-muted-foreground">
                {t("component.properties.additionalLegacyWorkspaces")}
              </div>
            )}
                    ...
                    <div className="min-w-0 space-y-0.5">
                      ...
                      <div className="text-[11px] text-muted-foreground">
                        {service.url ? (
                          ...
                          service.command ?? t("component.properties.noUrl")
                        )}
                      </div>
                    </div>
            ...
            <div className="space-y-1.5 rounded-md border border-border p-2">
              <div className="flex items-center gap-2">
                <input
                  ...
                  placeholder={t("component.properties.localFolderPathPlaceholder")}
                />
                <ChoosePathButton />
              </div>
              <div className="flex items-center gap-2">
          ...
          {workspaceMode === "repo" && (
            <div className="space-y-1.5 rounded-md border border-border p-2">
              <input
                ...
                placeholder={t("component.properties.repoUrlPlaceholder")}
              />
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
          ...
          {createWorkspace.isError && (
            <p className="text-xs text-destructive">{t("component.properties.failedToSaveWorkspace")}</p>
          )}
          {removeWorkspace.isError && (
            <p className="text-xs text-destructive">{t("component.properties.failedToDeleteWorkspace")}</p>
          )}
          {updateWorkspace.isError && (
            <p className="text-xs text-destructive">{t("component.properties.failedToUpdateWorkspace")}</p>
          )}
        </div>
        ...
        {isolatedWorkspacesEnabled ? (
          <>
            ...
            <div className="py-1.5 space-y-2">
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <span>{t("component.properties.executionWorkspaces")}</span>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      type="button"
                      className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-border text-[10px] text-muted-foreground hover:text-foreground"
                      aria-label={t("component.properties.executionWorkspacesHelp")}
                    >
                      ?
                    </button>
                  </TooltipTrigger>
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  {onUpdate || onFieldUpdate ? (
                    ...
                    <span className="text-xs text-muted-foreground">
                      {executionWorkspacesEnabled ? t("common.status.active") : t("common.status.disabled")}
                    </span>
                  )}
                </div>
                ...
                {executionWorkspacesEnabled ? (
                  <div className="space-y-3">
                    ...
                    <div className="border-t border-border/60 pt-2">
                      ...
                      >
                        {executionWorkspaceAdvancedOpen
                          ? t("component.properties.hideAdvancedCheckoutSettings")
                          : t("component.properties.showAdvancedCheckoutSettings")}
                      </button>
                    </div>
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                        <div className="text-xs text-muted-foreground">
                          {t("component.properties.hostManagedImplementation")} <span className="text-foreground">{t("component.properties.gitWorktree")}</span>
                        </div>
                        {environmentsEnabled ? (
                          <div>
                            <div className="mb-1 flex items-center gap-1.5">
                              <label className="flex items-center gap-2 text-xs text-muted-foreground">
                                <span>{t("component.properties.environment")}</span>
                            ...
                            >
                              <option value="">{t("component.properties.noEnvironment")}</option>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("component.properties.baseRef")}</span>
                          ...
                          <DraftInput
                            ...
                            immediate
                            className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs font-mono outline-none"
                            placeholder={t("component.properties.baseRefPlaceholder")}
                          />
                        </div>
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("component.properties.branchTemplate")}</span>
                          ...
                          <DraftInput
                            ...
                            immediate
                            className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs font-mono outline-none"
                            placeholder={t("component.properties.branchTemplatePlaceholder")}
                          />
                        </div>
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("component.properties.worktreeParentDir")}</span>
                          ...
                          <DraftInput
                            ...
                            immediate
                            className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs font-mono outline-none"
                            placeholder={t("component.properties.worktreeParentDirPlaceholder")}
                          />
                        </div>
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("component.properties.provisionCommand")}</span>
                          ...
                          <DraftInput
                            ...
                            immediate
                            className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs font-mono outline-none"
                            placeholder={t("component.properties.provisionCommandPlaceholder")}
                          />
                        </div>
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("component.properties.teardownCommand")}</span>
                          ...
                          <DraftInput
                            ...
                            immediate
                            className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs font-mono outline-none"
                            placeholder={t("component.properties.teardownCommandPlaceholder")}
                          />
                        </div>
                        <p className="text-[11px] text-muted-foreground">
                          {t("component.properties.provisionTeardownDescription")}
                        </p>
                      </div>
                    ) : null}
                  </div>
        ...
        <>
          ...
          <div className="space-y-4 py-4">
            <div className="text-xs font-medium text-destructive uppercase tracking-wide">
              {t("component.properties.dangerZone")}
            </div>
            <ArchiveDangerZone
              project={project}
              onArchive={onArchive}
