...
const PROJECT_STATUSES = [
  { value: "backlog", labelKey: "statusBacklog", labelDefault: "Backlog" },
  { value: "planned", labelKey: "statusPlanned", labelDefault: "Planned" },
  { value: "in_progress", labelKey: "statusInProgress", labelDefault: "In Progress" },
  { value: "completed", labelKey: "statusCompleted", labelDefault: "Completed" },
  { value: "cancelled", labelKey: "statusCancelled", labelDefault: "Cancelled" },
];
...
function SaveIndicator({ state }: { state: ProjectFieldSaveState }) {
  const { t } = useTranslation("projectProperties");
  if (state === "saving") {
    return (
...
function ArchiveDangerZone({
  ...
  const actionProjectLabel = isArchive ? t("archiveProject", "Archive project") : t("unarchiveProject", "Unarchive project");

  return (
    <div className="space-y-3 rounded-md border border-destructive/40 bg-destructive/5 px-4 py-4">
      <p className="text-sm text-muted-foreground">
        {isArchive
          ? t("archiveHint", "Archive this project to hide it from the sidebar and project selectors.")
          : t("unarchiveHint", "Unarchive this project to restore it in the sidebar and project selectors.")}
      </p>
      {archivePending ? (
        <Button size="sm" variant="destructive" disabled>
          <Loader2 className="h-3 w-3 animate-spin mr-1" />
          {isArchive ? t("archiving", "Archiving...") : t("unarchiving", "Unarchiving...")}
        </Button>
      ) : confirming ? (
        <div className="flex items-center gap-2">
          <span className="text-sm text-destructive font-medium">
            {isArchive
              ? t("confirmArchive", "Archive “{{name}}”?", { name: project.name })
              : t("confirmUnarchive", "Unarchive “{{name}}”?", { name: project.name })}
          </span>
          <Button
            size="sm"
            variant="destructive"
        ...
        >
          {isArchive ? (
            <><Archive className="h-3 w-3 mr-1" />{actionProjectLabel}</>
          ) : (
            <><ArchiveRestore className="h-3 w-3 mr-1" />{actionProjectLabel}</>
          )}
        </Button>
      )}
    </div>
...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
  ...
  const submitRepoWorkspace = () => {
    ...
    if (!looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(t("repoMustUseValidUrl", "Repo must use a valid GitHub or GitHub Enterprise repo URL."));
      return;
    }
    setWorkspaceError(null);
  ...
  const clearLocalWorkspace = () => {
    const confirmed = window.confirm(
      codebase.repoUrl
        ? t("clearLocalFolderConfirm", "Clear local folder from this workspace?")
        : t("deleteLocalFolderConfirm", "Delete this workspace local folder?"),
    );
    if (!confirmed) return;
    persistCodebase({ cwd: null });
  };
  ...
  const clearRepoWorkspace = () => {
    ...
    const confirmed = window.confirm(
      hasLocalFolder
        ? t("clearRepoConfirm", "Clear repo from this workspace?")
        : t("deleteRepoConfirm", "Delete this workspace repo?"),
    );
    if (!confirmed) return;
  ...
  return (
    <div>
      <div className="space-y-1 pb-4">
        <PropertyRow label={<FieldLabel label={t("name", "Name")} state={fieldState("name")} />}>
        ...
        <PropertyRow
          label={<FieldLabel label={t("description", "Description")} state={fieldState("description")} />}
          alignStart
        ...
        >
          {onUpdate || onFieldUpdate ? (
            ...
            <p className="text-sm text-muted-foreground">
              {project.description?.trim() || t("noDescription", "No description")}
            </p>
          )}
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("status", "Status")} state={fieldState("status")} />}>
          {onUpdate || onFieldUpdate ? (
            <ProjectStatusPicker
        ...
        {project.leadAgentId && (
          <PropertyRow label={t("lead", "Lead")}>
            <span className="text-sm font-mono">{project.leadAgentId.slice(0, 8)}</span>
          </PropertyRow>
        )}
        <PropertyRow
          label={<FieldLabel label={t("goals", "Goals")} state={fieldState("goals")} />}
          alignStart
              ...
              <PopoverContent className="w-56 p-1" align="start">
                {availableGoals.length === 0 ? (
                  <div className="px-2 py-1.5 text-xs text-muted-foreground">
                    {t("allGoalsLinked", "All goals linked.")}
                  </div>
                ) : (
        ...
        <PropertyRow
          label={<FieldLabel label={t("env", "Env")} state={fieldState("env")} />}
          alignStart
          valueClassName="space-y-2"
        >
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-muted-foreground">
              {t("envHint", "Applied to all runs for issues in this project. Project values override agent env on key conflicts.")}
            </p>
          </div>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("created", "Created")} state="idle" />}>
          <span className="text-sm">{formatDate(project.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("updated", "Updated")} state="idle" />}>
          <span className="text-sm">{formatDate(project.updatedAt)}</span>
        </PropertyRow>
        {project.targetDate && (
          <PropertyRow label={<FieldLabel label={t("targetDate", "Target Date")} state="idle" />}>
      ...
      <div className="space-y-1 py-4">
        <div className="space-y-2">
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <span>{t("codebase", "Codebase")}</span>
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  type="button"
                  className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-border text-[10px] text-muted-foreground hover:text-foreground"
                  aria-label={t("codebaseHelp", "Codebase help")}
                >
                  ?
                </button>
              </TooltipTrigger>
              <TooltipContent side="top">
                {t("codebaseHelpText", "Repo identifies the source of truth. Local folder is the default place agents write code.")}
              </TooltipContent>
            </Tooltip>
          </div>
          <div className="space-y-2 rounded-md border border-border/70 p-3">
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("repo", "Repo")}</div>
              {codebase.repoUrl ? (
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    >
                      {t("changeRepo", "Change repo")}
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon-xs"
                      onClick={clearRepoWorkspace}
                      aria-label={t("clearRepo", "Clear repo")}
                    >
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="text-xs text-muted-foreground">{t("notSet", "Not set.")}</div>
                  <Button
                    variant="outline"
                    size="xs"
                    className="h-6 px-2"
            ...
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("localFolder", "Local folder")}</div>
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0 space-y-1">
                  ...
                  {codebase.origin === "managed_checkout" && (
                    <div className="text-[11px] text-muted-foreground">{t("paperclipManagedFolder", "Paperclip-managed folder.")}</div>
                  )}
                </div>
                <div className="flex items-center gap-1">
                  <Button
                  ...
                  >
                    {codebase.localFolder ? t("changeLocalFolder", "Change local folder") : t("setLocalFolder", "Set local folder")}
                  </Button>
                  {codebase.localFolder ? (
                    <Button
                      ...
                      aria-label={t("clearLocalFolder", "Clear local folder")}
                    >
                      <Trash2 className="h-3 w-3" />
                    </Button>
                  ) : null}
            ...
            {hasAdditionalLegacyWorkspaces && (
              <div className="text-[11px] text-muted-foreground">
                {t("legacyWorkspacesHint", "Additional legacy workspace records exist on this project. Paperclip is using the primary workspace as the codebase view.")}
              </div>
            )}
                    ...
                    <div className="min-w-0 space-y-0.5">
                      ...
                      <div className="text-[11px] text-muted-foreground">
                        {service.url ? (
                          ...
                          service.command ?? t("noUrl", "No URL")
                        )}
                      </div>
                    </div>
          ...
          {createWorkspace.isError && (
            <p className="text-xs text-destructive">{t("failedToSaveWorkspace", "Failed to save workspace.")}</p>
          )}
          {removeWorkspace.isError && (
            <p className="text-xs text-destructive">{t("failedToDeleteWorkspace", "Failed to delete workspace.")}</p>
          )}
          {updateWorkspace.isError && (
            <p className="text-xs text-destructive">{t("failedToUpdateWorkspace", "Failed to update workspace.")}</p>
          )}
        </div>
        ...
        {isolatedWorkspacesEnabled ? (
          <>
            ...
            <div className="py-1.5 space-y-2">
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <span>{t("executionWorkspaces", "Execution Workspaces")}</span>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      type="button"
                      className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-border text-[10px] text-muted-foreground hover:text-foreground"
                      aria-label={t("executionWorkspacesHelp", "Execution workspaces help")}
                    >
                      ?
                    </button>
                  </TooltipTrigger>
                  <TooltipContent side="top">
                    {t("executionWorkspacesHelpText", "Project-owned defaults for isolated issue checkouts and execution workspace behavior.")}
                  </TooltipContent>
                </Tooltip>
              </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2 text-sm font-medium">
                      <span>{t("enableIsolatedCheckouts", "Enable isolated issue checkouts")}</span>
                      <SaveIndicator state={fieldState("execution_workspace_enabled")} />
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {t("enableIsolatedCheckoutsHint", "Let issues choose between the project's primary checkout and an isolated execution workspace.")}
                    </div>
                  </div>
                  {onUpdate || onFieldUpdate ? (
                    <ToggleSwitch
                    ...
                    <span className="text-xs text-muted-foreground">
                      {executionWorkspacesEnabled ? t("enabled", "Enabled") : t("disabled", "Disabled")}
                    </span>
                  )}
                </div>
                ...
                {executionWorkspacesEnabled ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2 text-sm">
                          <span>{t("newIssuesDefaultIsolated", "New issues default to isolated checkout")}</span>
                          <SaveIndicator state={fieldState("execution_workspace_default_mode")} />
                        </div>
                        <div className="text-[11px] text-muted-foreground">
                          {t("newIssuesDefaultIsolatedHint", "If disabled, new issues stay on the project's primary checkout unless someone opts in.")}
                        </div>
                      </div>
                      <ToggleSwitch
                    ...
                    <div className="border-t border-border/60 pt-2">
                      ...
                      >
                        {executionWorkspaceAdvancedOpen
                          ? t("hideAdvanced", "Hide advanced checkout settings")
                          : t("showAdvanced", "Show advanced checkout settings")}
                      </button>
                    </div>
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                        <div className="text-xs text-muted-foreground">
                          {t("hostManagedImplementation", "Host-managed implementation:")} <span className="text-foreground">{t("gitWorktree", "Git worktree")}</span>
                        </div>
                        {environmentsEnabled ? (
                          <div>
                            ...
                            >
                              <option value="">{t("noEnvironment", "No environment")}</option>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("branchTemplate", "Branch template")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("worktreeParentDir", "Worktree parent dir")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("provisionCommand", "Provision command")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("teardownCommand", "Teardown command")}</span>
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          {t("provisionTeardownHint", "Provision runs inside the derived worktree before agent execution. Teardown is stored here for future cleanup flows.")}
                        </p>
                      </div>
                    ) : null}
                  </div>
